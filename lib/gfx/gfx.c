...
static void copyrect8(gfx_surface *surface, uint x, uint y, uint width, uint height, uint x2,
                      uint y2) {
  // copy
...
static void copyrect16(gfx_surface *surface, uint x, uint y, uint width, uint height, uint x2,
                       uint y2) {
  // copy
...
static void copyrect32(gfx_surface *surface, uint x, uint y, uint width, uint height, uint x2,
                       uint y2) {
  // copy
 ...
 */
gfx_surface *gfx_create_surface(void *ptr, uint width, uint height, uint stride,
                                gfx_format format) {
...
gfx_surface *gfx_create_surface_from_display(struct display_framebuffer *fb) {
  ...
  surface = gfx_create_surface(fb->image.pixels, fb->image.width, fb->image.height,
                               fb->image.stride, format);
  ...
  if (!strcmp(argv[1].str, "display_info")) {
    ...
    for (y = 0; y < surface->height; y++) {
      for (x = 0; x < surface->width; x++) {
        ...
        gfx_putpixel(surface, x, y,
                     (0xff << 24) | (argv[2].i << 16) | (argv[3].i << 8) | argv[4].i);
      }
    }
