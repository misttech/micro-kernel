...
#define DEFINE_STDIO_DESC(id) [(id)] = {.io = &console_io, STDIO_FIELDS}
...
static ssize_t fread_error(void *ptr, size_t size, size_t count, FILE *stream) {
  ...
  if (stream->use_fs) {
    ssize_t rsize =
        fs_read_file(stream->fs_handle.handle, ptr, stream->fs_handle.offset, size * count);
    if (rsize <= 0) {
      return rsize;
    }
...
static ssize_t fwrite_error(const void *ptr, size_t size, size_t count, FILE *stream) {
  ...
  if (stream->use_fs) {
    ...
    ssize_t wsize =
        fs_write_file(stream->fs_handle.handle, ptr, stream->fs_handle.offset, size * count);
    if (wsize <= 0) {
      return wsize;
    }
