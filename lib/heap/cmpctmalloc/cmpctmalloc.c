...
static void lock(void) { mutex_acquire(&theheap.lock); }

static void unlock(void) { mutex_release(&theheap.lock); }
...
static inline header_t *tag_as_free(void *left) { return (header_t *)((uintptr_t)left | 1); }

static inline bool is_tagged_as_free(header_t *header) {
  return ((uintptr_t)(header->left) & 1) != 0;
}

static inline header_t *untag(void *left) { return (header_t *)((uintptr_t)left & ~1); }
...
static bool is_start_of_os_allocation(header_t *header) { return header->left == untag(NULL); }
...
static bool is_end_of_os_allocation(char *address) { return ((header_t *)address)->size == 0; }
...
void *cmpct_alloc(size_t size) {
  ...
  if (bucket == -1) {
    ...
    size_t growby =
        MIN(1u << HEAP_ALLOC_VIRTUAL_BITS, MAX(theheap.size >> 3, MAX(HEAP_GROW_SIZE, rounded_up)));
