...
static void ubsan_abort() { panic("ubsan!\n"); }
...
__USED void __ubsan_handle_pointer_overflow(ubsan_ptr_overflow_data *data, size_t Base,
                                            size_t Result) {
...
static void ubsan_handle_integer_overflow(const ubsan_overflow_data *data, size_t lhs, size_t rhs,
  ...
  printf("%s integer overflow: %s %s %s can't be represented in type %s",
         signed_ ? "signed" : "unsigned", lhs_v.to_string().c_str(), op, rhs_v.to_string().c_str(),
         data->type->typename_);
  ubsan_report_end();
}
...
__USED void __ubsan_handle_divrem_overflow_abort(ubsan_overflow_data *data, size_t lhs,
                                                 size_t rhs) {
