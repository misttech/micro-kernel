...
class ScopeGuard {
 public:
  ...
  constexpr ScopeGuard(ScopeGuard &&that) noexcept : f_(that.f_), active_(that.active_) {
    that.active_ = false;
  }

  template <typename Functor>
  constexpr ScopeGuard(ScopeGuard<Functor> &&that) : f_(that.f_), active_(that.active_) {
    that.active_ = false;
  }
...
#define DEFER auto TOKENPASTE2(_deferred_lambda_call, __COUNTER__) = deferrer << [&]() mutable
