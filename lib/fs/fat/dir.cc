...
status_t fat_find_next_entry(fat_fs *fat, file_block_iterator &dbi, uint32_t &offset,
                             dir_entry *entry, char filename_buffer[MAX_FILE_NAME_LEN],
                             char **out_filename) {
  DEBUG_ASSERT(entry && filename_buffer && out_filename);
  DEBUG_ASSERT(offset <=
               fat->info().bytes_per_sector);  // passing offset == bytes_per_sector is okay
...
status_t fat_walk(fat_fs *fat, const char *path, dir_entry *out_entry, dir_entry_location *loc) {
  ...
  for (;;) {
    ...
    // push the name element forward one notch so we can see if we're at the end (or iterate once
    // again)
    path_increment();
...
status_t fat_dir::opendir_priv(const dir_entry &entry, const dir_entry_location &loc,
                               fat_dir_cookie **out_cookie) {
...
status_t fat_dir::readdir_priv(fat_dir_cookie *cookie, struct dirent *ent) {
...

  {
    ...
    // reset how many sectors the dbi has pushed forward so we can account properly for index shifts
    // later
...
status_t fat_dir::closedir(dircookie *dcookie) {
  ...
  if (last_ref) {
    LTRACEF("last ref, deleting %p (%u:%u)\n", dir, dir->dir_loc().starting_dir_cluster,
            dir->dir_loc().dir_offset);
    delete dir;
  }
