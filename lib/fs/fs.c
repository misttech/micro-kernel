...
status_t fs_file_ioctl(filehandle *handle, int request, void *argp) {
  ...
  if (unlikely(!handle || !handle->mount || !handle->mount->api ||
               !handle->mount->api->file_ioctl)) {
...
void fs_normalize_path(char *path) {
  ...
  /* remove duplicate path separators, flatten empty fields (only composed of .), backtrack fields
   * with .., remove trailing slashes */
  while (!done) {
    c = path[pos];
    switch (state) {
      case INITIAL:
