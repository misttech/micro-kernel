...
static void send_error(udp_socket_t *socket, uint16_t code) {
  ...
  uint16_t err[] = {htons(TFTP_OPCODE_ERROR), ncode, 0x7245, 0x2072, 0x3030 + ncode, 0};
...
static void udp_wrq_callback(void *data, size_t len, uint32_t srcaddr, uint16_t srcport,
                             void *arg) {
...
static void udp_svc_callback(void *data, size_t len, uint32_t srcaddr, uint16_t srcport,
                             void *arg) {
  status_t st;
  uint16_t opcode;
  udp_socket_t *socket;
