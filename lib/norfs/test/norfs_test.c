...
static const unsigned char good_object[16] = {2, 0,    0,    0,    0, 0, 4, 0,
                                              0, 0xFF, 0x3d, 0x02, 0, 1, 2, 3};
/* Object with a good checksum. key = 2; version = 1; obj = {5, 4, 3, 2} */
static const unsigned char good_object_v1[16] = {2, 0,    0,    0,    1, 0, 4, 0,
                                                 0, 0xFF, 0xbb, 0x9d, 5, 4, 3, 2};
/* A much later version of good_object to test wrapping of versions. */
static const unsigned char good_object_v127[16] = {2, 0,    0,    0,    0xFF, 0x7F, 4, 0,
                                                   0, 0xFF, 0x76, 0x3c, 6,    7,    8, 9};
/* A much later version of good_object to test wrapping of versions. */
static const unsigned char good_object_v128[16] = {2, 0,    0,    0,    0x00, 0x80, 4, 0,
                                                   0, 0xFF, 0x04, 0x24, 6,    7,    8, 9};
/* Deleted object. key = 2; version = 2 */
static const unsigned char good_object_deleted_v2[12] = {2, 0, 0, 0,    2,    0,
                                                         0, 0, 1, 0xFF, 0xb7, 0x8a};
/* Deleted object. key = 2; version = 3 */
static const unsigned char good_object_deleted_v3[12] = {2, 0, 0, 0,    3,    0,
                                                         0, 0, 1, 0xFF, 0xe6, 0x20};
/* Object with a bad checksum. key = 4; version = 2; obj = {3, 2, 1, 0} */
static const unsigned char bad_object[16] = {4, 0, 0, 0, 2, 0, 4, 0, 0, 0xFF, 4, 7, 3, 2, 1, 0};
/* Object with a good checksum. key = 0; version = 0; obj = {9, 8, 7, 6} */
static const unsigned char another_good_object[16] = {0, 0,    0,    0,    0, 0, 4, 0,
                                                      0, 0xFF, 0x4d, 0x4a, 9, 8, 7, 6};
...
static void write_block_header(uint *ptr) {
  *ptr += flash_nor_write(0, *ptr, sizeof(NORFS_BLOCK_HEADER), NORFS_BLOCK_HEADER);
...
static bool test_basic_read_write(void) {
  ...
  EXPECT_EQ(ERR_NOT_MOUNTED, norfs_put_obj(char_key, char_array, sizeof(char_array), 0),
            "Calls to file system should fail when not mounted");
  EXPECT_EQ(ERR_NOT_MOUNTED,
            norfs_read_obj(char_key, char_array_buffer, sizeof(unsigned char) * 6,
                           &char_array_bytes_read, 0),
            "Calls to file system should fail when not mounted");
  EXPECT_EQ(ERR_NOT_MOUNTED, norfs_put_obj_iovec(17, iov_in, iov_in_count, 0),
            "Calls to file system should fail when not mounted");
  EXPECT_EQ(ERR_NOT_MOUNTED, norfs_read_obj_iovec(17, iov_out, iov_out_count, &obj_len, 0),
            "Calls to file system should fail when not mounted");
  ...
  status = norfs_read_obj(int_key, (unsigned char *)&big_int_buffer, sizeof(big_int_buffer),
                          &big_int_bytes_read, 0);
  EXPECT_EQ(NO_ERROR, status, "norfs_read_obj returned error");
  if (status)
    return false;

  if (!status) {
    EXPECT_EQ(sizeof(char_array), char_array_bytes_read, "Not all bytes of object read");
    EXPECT_EQ(sizeof(big_int), big_int_bytes_read, "Not all bytes of object read");
    for (int i = 0; i < 5; i++) {
      EXPECT_EQ(char_array[i], char_array_buffer[i], "Did not read object from NVRAM correctly");
    }

    EXPECT_EQ(big_int, big_int_buffer, "Did not read object from NVRAM correctly");
  }
  ...
  if (!status) {
    EXPECT_EQ('a', *((unsigned char *)iov_out[0].iov_base), "Iovec not retrieved correctly");
    EXPECT_EQ('b', *((unsigned char *)iov_out[1].iov_base), "Iovec not retrieved correctly");
    EXPECT_EQ('c', *((unsigned char *)iov_out[1].iov_base + 1), "Iovec not retrieved correctly");
    EXPECT_EQ('d', *((unsigned char *)iov_out[1].iov_base + 2), "Iovec not retrieved correctly");
    EXPECT_EQ('e', *((unsigned char *)iov_out[2].iov_base), "Iovec not retrieved correctly");
  } else {
    return false;
  }
  ...
  if (!status) {
    EXPECT_EQ(char_array_bytes_read, sizeof(char_array), "Not all bytes of object read");
...
static bool test_garbage_collection(void) {
  ...
  EXPECT_EQ(NO_ERROR, status, "Error reading object");
...
static bool test_total_remaining_space(void) {
  ...
  EXPECT_EQ(total_remaining_space, prev_remaining_space, "Remaining space incorrectly maintained");
  wipe_fs();
  END_TEST;
}
...
static bool test_deletion(void) {
  ...
  ptr += flash_nor_write(0, ptr, sizeof(good_object_deleted_v2), good_object_deleted_v2);

  EXPECT_EQ(NO_ERROR, norfs_mount_fs(norfs_nvram_offset), "Error during mount");
  status = get_inode(2, &inode);
  EXPECT_EQ(true, status, "Inode should be in list since references remain");
  if (!status)
    return false;
  ...
  write_pointer += flash_nor_write(0, write_pointer, sizeof(erase_header), erase_header);
  EXPECT_EQ(NO_ERROR, collect_block(0, &write_pointer), "Error when collecting block");
  ...
  ptr += flash_nor_write(0, ptr, sizeof(good_object_deleted_v2), good_object_deleted_v2);
  EXPECT_EQ(NO_ERROR, norfs_mount_fs(norfs_nvram_offset), "Error during mount");
  EXPECT_EQ(true, get_inode(2, &inode), "Inode should be in list");
...
static bool test_corruption(void) {
  ...
  EXPECT_EQ(NO_ERROR, status, "Object prior to corruption should still be readable");

  /* good_object_v1, appearing after the corruption, should be discarded. */
  for (int i = 0; i < 4; i++) {
    EXPECT_EQ(i, buff[i], "First version of object not correctly retrieved");
  }
  ...
  EXPECT_EQ(ERR_NOT_FOUND, status, "Object after corrupted object should not be found");
...
static bool test_deletion_objects_retained_during_gc(void) {
  ...
  uint16_t obj_len = (FLASH_PAGE_SIZE - BLOCK_HEADER_SIZE) / 20 - OBJ_HEADER_LENGTH;
  status_t status;
  ...
  status = norfs_read_obj(key, obj, obj_len, &bytes_read, 0);
  EXPECT_EQ(ERR_NOT_FOUND, status, "Error: Object removal lost after garbage collection");
...
static bool test_garbage_collection_interruption(void) {
  ...
  ptr += flash_nor_write(0, ptr, sizeof(NORFS_BLOCK_HEADER), NORFS_BLOCK_HEADER);
  ptr +=
      flash_nor_write(0, ptr, sizeof(NORFS_BLOCK_GC_STARTED_HEADER), NORFS_BLOCK_GC_STARTED_HEADER);
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    switch (action) {
      case 0:
        status = norfs_remove_obj(key);
        if (state[key] == -1) {
          EXPECT_EQ(ERR_NOT_FOUND, status, "Removed object not returning correctly from read");
        } else {
          EXPECT_EQ(NO_ERROR, status, "Object not returning correctly from remove");
          if (status) {
            dump_bank();
            return false;
          }
      ...
      case 1:
        ...
        EXPECT_EQ(NO_ERROR, status, "Object not returning correctly from put");
        state[key] = i;
        break;
      case 2:
      ...
      case 3:
        ...
        if (state[key] == -1) {
          ...
          EXPECT_EQ(NO_ERROR, status, "Object not returning correctly from read");
        ...
        break;
      case 4:
        status = norfs_read_obj(key, (unsigned char *)&int_buff, sizeof(int), &bytes_read, 0);
        if (state[key] == -1) {
          EXPECT_EQ(ERR_NOT_FOUND, status, "Removed object not returning correctly from read");
        } else {
          EXPECT_EQ(NO_ERROR, status, "Object not returning correctly from read");
    ...
    EXPECT_GE(8 * FLASH_PAGE_SIZE - 1, write_pointer, "Write pointer has overflowed NVRAM");
  }
  for (int i = 0; i < 16; i++) {
