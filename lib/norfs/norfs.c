...
FRIEND_TEST uint8_t block_num(uint32_t flash_pointer) { return flash_pointer / FLASH_PAGE_SIZE; }
...
static uint8_t select_garbage_block(uint32_t ptr) { return (block_num(ptr) + 1) % 8; }
...
static ssize_t nvram_write(size_t offset, size_t length, const void *ptr) {
  return flash_nor_write(NORFS_BANK, offset + norfs_nvram_offset, length, ptr);
}

static ssize_t nvram_erase_pages(size_t offset, size_t length) {
  return flash_nor_erase_pages(NORFS_BANK, offset + norfs_nvram_offset, length);
}
...
FRIEND_TEST bool get_inode(uint32_t key, struct norfs_inode **inode) {
  ...
  *inode = NULL;
  list_for_every(&inode_list, curr_lnode) {
    ...
    nvram_read(curr_inode->location + NORFS_KEY_OFFSET, sizeof(curr_key), &curr_key);
    if (curr_key == key) {
      *inode = curr_inode;
      return true;
    }
...
static uint16_t calculate_header_crc(uint32_t key, uint16_t version, uint16_t len, uint8_t flags) {
...
static ssize_t read_header(uint32_t ptr, struct norfs_header *header) {
  ...
  bytes_read = nvram_read(ptr + NORFS_KEY_OFFSET, sizeof(header->key), &header->key);
  if (bytes_read < 0) {
    return bytes_read;
  }
  total_bytes_read += bytes_read;
  bytes_read = nvram_read(ptr + NORFS_VERSION_OFFSET, sizeof(header->version), &header->version);
  if (bytes_read < 0) {
    return bytes_read;
  }
  total_bytes_read += bytes_read;
  bytes_read = nvram_read(ptr + NORFS_LENGTH_OFFSET, sizeof(header->len), &header->len);
  if (bytes_read < 0) {
    return bytes_read;
  }
  total_bytes_read += bytes_read;
  bytes_read = nvram_read(ptr + NORFS_FLAGS_OFFSET, sizeof(header->flags), &header->flags);
  if (bytes_read < 0) {
    return bytes_read;
  }
  total_bytes_read += bytes_read;
  ...
  total_bytes_read += 1;
  bytes_read = nvram_read(ptr + NORFS_CHECKSUM_OFFSET, sizeof(header->crc), &header->crc);
  if (bytes_read < 0) {
    return bytes_read;
  }
  total_bytes_read += bytes_read;
...
status_t norfs_read_obj_iovec(uint32_t key, iovec_t *obj_iov, uint32_t iov_count,
  ...
  while (*bytes_read < total_to_read) {
    ...
    bytes = nvram_read(read_ptr + *bytes_read, to_read, obj_iov[i].iov_base);
    if (bytes < 0) {
...
static status_t write_obj_header(uint32_t *ptr, uint32_t key, uint16_t version, uint16_t len,
                                 uint8_t flags, uint16_t crc) {
...
static status_t copy_iovec_to_disk(const struct iovec *iov, uint16_t iov_count, uint32_t *location,
                                   uint16_t *crc) {
  ...
  for (uint16_t i = 0; i < iov_count; i++) {
    while ((uint16_t)iov[i].iov_len - iov_ptr > word_size - word_ptr) {
      memcpy(word + word_ptr, (unsigned char *)iov[i].iov_base + iov_ptr, word_size - word_ptr);
      iov_ptr += word_size - word_ptr;

      bytes_written = nvram_write(*location, sizeof(word), &word);
      if (bytes_written < 0) {
    ...
    memcpy((unsigned char *)word + word_ptr, (unsigned char *)iov[i].iov_base + iov_ptr,
...
static status_t initialize_next_block(uint32_t *ptr) {
  ...
  bytes_written =
      nvram_write(*ptr, sizeof(NORFS_BLOCK_GC_STARTED_HEADER), &NORFS_BLOCK_GC_STARTED_HEADER);
  ...
  *ptr += sizeof(NORFS_BLOCK_GC_STARTED_HEADER) + sizeof(NORFS_BLOCK_GC_FINISHED_HEADER);
  ...
  status = nvram_write(header_pointer, sizeof(NORFS_BLOCK_GC_FINISHED_HEADER),
...
status_t write_obj_iovec(const iovec_t *iov, uint iov_count, uint *location, uint32_t key,
...
status_t norfs_put_obj(uint32_t key, unsigned char *obj, uint16_t len, uint8_t flags) {
  if (!fs_mounted)
    return ERR_NOT_MOUNTED;
...
status_t norfs_remove_obj(uint32_t key) {
  ...
  status = nvram_read(inode->location + NORFS_LENGTH_OFFSET, sizeof(uint16_t), &prior_len);
  if (status < 0) {
 ...
 */
status_t norfs_put_obj_iovec(uint32_t key, const iovec_t *iov, uint32_t iov_count, uint8_t flags) {
  if (!fs_mounted)
    return ERR_NOT_MOUNTED;
  ...
  if (obj_preexists) {
    nvram_read(inode->location + NORFS_FLAGS_OFFSET, sizeof(stored_flags), &stored_flags);
    if (stored_flags & flags & NORFS_DELETED_MASK) {
      /* Attempting to delete an object no longer in filesystem. */
      TRACEF(
          "The object attempting to be removed has already been \
			deleted. stored_flags: 0x%x\n",
          stored_flags);
      return ERR_NOT_FOUND;
    }

    nvram_read(inode->location + NORFS_VERSION_OFFSET, sizeof(version), &version);
    version++;
  } else if (deletion) {
  ...
  if (!deletion && (NORFS_FLASH_SIZE(len) > total_remaining_space)) {
    TRACEF(
        "Not enough remaining space.  Remaining space:  %d\tObject len: \
			    %d\n",
  ...
  status = write_obj_iovec(iov, iov_count, &write_pointer, key, version, flags);
  if (!status) {
    if (!obj_preexists) {
      ...
      uint16_t prior_len;
      nvram_read(inode->location + NORFS_LENGTH_OFFSET, sizeof(uint16_t), &prior_len);
...
static status_t collect_garbage_object(uint32_t *garbage_read_pointer,
  ...
  if (inode_found) {
    if (garb_obj_loc == inode->location) {
      ...
      status = write_obj_iovec(iov, 1, garbage_write_pointer, header.key, header.version + 1,
                               header.flags);
      if (status) {
...
static status_t erase_block(uint8_t block) {
  ...
  if (bytes_erased != FLASH_PAGE_SIZE) {
    flash_nor_end(NORFS_BANK);
    TRACEF(
        "Did not erase exactly one flash page.  Something went \
				wrong.\n");
    return ERR_IO;
  }

  bytes_written = nvram_write(loc, sizeof(NORFS_BLOCK_HEADER), &NORFS_BLOCK_HEADER);
...
FRIEND_TEST status_t collect_block(uint32_t garbage_block, uint32_t *garbage_write_ptr) {
  status_t status;
  uint32_t garbage_read_ptr = garbage_block * FLASH_PAGE_SIZE + NORFS_BLOCK_HEADER_SIZE;
...
static status_t load_and_verify_obj(uint32_t *ptr, struct norfs_header *header) {
  ...
  calculated_crc = calculate_header_crc(header->key, header->version, header->len, header->flags);
...
status_t read_block_verification(uint32_t *ptr) {
  unsigned char block_header[sizeof(NORFS_BLOCK_HEADER) + sizeof(NORFS_BLOCK_GC_STARTED_HEADER) +
                             sizeof(NORFS_BLOCK_GC_FINISHED_HEADER)];
  int bytes_read = nvram_read(*ptr, sizeof(block_header), block_header);
  ...
  for (uint8_t i = sizeof(NORFS_BLOCK_HEADER);
       i < sizeof(NORFS_BLOCK_HEADER) + sizeof(NORFS_BLOCK_GC_STARTED_HEADER) +
               sizeof(NORFS_BLOCK_GC_FINISHED_HEADER);
       i++) {
...
static status_t mount_next_obj(void) {
  ...
  if (get_inode(header.key, &inode)) {
    nvram_read(inode->location + NORFS_VERSION_OFFSET, sizeof(inode_version), &inode_version);
    if (VERSION_GREATER_THAN(header.version, inode_version)) {
      ...
      nvram_read(inode->location + NORFS_LENGTH_OFFSET, sizeof(inode_len), &inode_len);
