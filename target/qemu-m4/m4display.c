...
static const char programming_header[] =
    "  Lattice\0iCEcube2 2014.08.26723\0Part: iCE40LP1K-CM36\0Date: Jan 30 2015 15:11:";
...
static void s4lcd_flush(uint starty, uint endy) {
  ...
  for (int i = 0; i < M4DISPLAY_HEIGHT; i++) {
    ...
    for (int j = 0; j < M4DISPLAY_WIDTH; j += 2) {
      *msb++ =
          ((framebuffer[i][j] & 0b01010100) | ((framebuffer[i][j + 1] & 0b01010100) << 1)) >> 2;
      *lsb++ =
          ((framebuffer[i][j] & 0b10101000) | ((framebuffer[i][j + 1] & 0b10101000) >> 1)) >> 2;
    }
