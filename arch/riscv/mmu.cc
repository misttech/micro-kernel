...
constexpr uint vaddr_to_index(vaddr_t va, uint level) {
  ...
  uint index =
      ((va >> PAGE_SIZE_SHIFT) >> (level * RISCV_MMU_PT_SHIFT)) & (RISCV_MMU_PT_ENTRIES - 1);
...
uintptr_t constexpr page_mask_per_level(uint level) { return page_size_per_level(level) - 1; }
...
constexpr uint kernel_end_index =
    vaddr_to_index(KERNEL_ASPACE_BASE + KERNEL_ASPACE_SIZE - 1UL, RISCV_MMU_PT_LEVELS - 1);
...
enum class walk_cb_ret_op { HALT, RESTART, ALLOC_PT };
...
struct walk_cb_ret {
  ...
  static walk_cb_ret OpCommitHalt(riscv_pte_t pte, bool unmap, int err) {
    return {walk_cb_ret_op::HALT, err, true, pte, unmap};
  }
  static walk_cb_ret OpCommitRestart(riscv_pte_t pte, bool unmap) {
    return {walk_cb_ret_op::RESTART, NO_ERROR, true, pte, unmap};
  }
...
int arch_mmu_map(arch_aspace_t *aspace, const vaddr_t _vaddr, paddr_t paddr, uint count,
                 const uint flags) {
  ...
  auto map_cb = [&paddr, &count, aspace, flags](uint level, uint index, riscv_pte_t pte,
                                                vaddr_t *vaddr) -> walk_cb_ret {
    LTRACEF("level %u, index %u, pte %#lx, vaddr %#lx [paddr %#lx count %u flags %#x]\n", level,
            index, pte, *vaddr, paddr, count, flags);
...
status_t arch_mmu_query(arch_aspace_t *aspace, const vaddr_t _vaddr, paddr_t *paddr, uint *flags) {
  ...
  auto query_cb = [paddr, flags](uint level, uint index, riscv_pte_t pte,
                                 vaddr_t *vaddr) -> walk_cb_ret {
...
int arch_mmu_unmap(arch_aspace_t *aspace, const vaddr_t _vaddr, const uint _count) {
  ...
  auto count = _count;
  auto unmap_cb = [&count](uint level, uint index, riscv_pte_t pte, vaddr_t *vaddr) -> walk_cb_ret {
    LTRACEF("level %u, index %u, pte %#lx, vaddr %#lx\n", level, index, pte, *vaddr);
...
extern "C" void riscv_mmu_init_secondaries() {
...
extern "C" void riscv_early_mmu_init() {
  ...
  for (auto i = kernel_start_index; i <= kernel_end_index; i++) {
    if ((trampoline_pgtable[i] & RISCV_PTE_V) == 0) {
      trampoline_pgtable[i] =
          RISCV_PTE_PPN_TO_PTE(kernel_l2_pgtable_phys + (i - kernel_start_index) * PAGE_SIZE) |
          RISCV_PTE_V;
    }
  }
...
extern "C" void riscv_mmu_init() { dprintf(INFO, "RISCV: MMU ASID mask %#lx\n", riscv_asid_mask); }
