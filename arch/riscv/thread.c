...
void arch_context_switch(thread_t *oldthread, thread_t *newthread) {
  ...
  LTRACEF("old fpu dirty %d, new fpu dirty %d, status %#lx, sd %d\n",
          ...
          hw_state >> RISCV_CSR_XSTATUS_FS_SHIFT, (status & RISCV_CSR_XSTATUS_SD) ? 1 : 0);
  ...
  if (newthread->arch.cs_frame.fpu_dirty) {
    ...
    /* TODO: see if it's totally safe to reduce to a single instruction based on moving from DIRTY
     * -> CLEAN */
