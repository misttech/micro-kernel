...
static bool map_region_query_result(vmm_aspace_t *aspace, uint arch_flags) {
  ...
  EXPECT_EQ(NO_ERROR,
            vmm_alloc(aspace, "test region", PAGE_SIZE, &ptr, 0, /* vmm_flags */ 0, arch_flags),
            "map region");
  ...
  // query the page to see if it's realistic
  {
    ...
    EXPECT_EQ(NO_ERROR, arch_mmu_query(&aspace->arch_aspace, (vaddr_t)ptr, &pa, &flags),
              "arch_query");
...
static bool map_query_pages(void) {
  ...
  // try mapping pages in the kernel address space with various permissions and read them back via
  // arch query
  ...
  if (arch_mmu_supports_nx_mappings()) {
    ...
    EXPECT_TRUE(
        map_region_query_result(kaspace, ARCH_MMU_FLAG_PERM_RO | ARCH_MMU_FLAG_PERM_NO_EXECUTE),
        "3");
  } else {
    EXPECT_TRUE(map_region_expect_failure(kaspace, ARCH_MMU_FLAG_PERM_NO_EXECUTE, ERR_INVALID_ARGS),
                "2");
    EXPECT_TRUE(
        map_region_expect_failure(kaspace, ARCH_MMU_FLAG_PERM_RO | ARCH_MMU_FLAG_PERM_NO_EXECUTE,
                                  ERR_INVALID_ARGS),
        "3");
  }

  EXPECT_TRUE(map_region_query_result(kaspace, ARCH_MMU_FLAG_PERM_USER), "4");
  EXPECT_TRUE(map_region_query_result(kaspace, ARCH_MMU_FLAG_PERM_USER | ARCH_MMU_FLAG_PERM_RO),
              "5");
  if (arch_mmu_supports_nx_mappings()) {
    EXPECT_TRUE(
        map_region_query_result(kaspace, ARCH_MMU_FLAG_PERM_USER | ARCH_MMU_FLAG_PERM_NO_EXECUTE),
        "6");
    EXPECT_TRUE(map_region_query_result(kaspace, ARCH_MMU_FLAG_PERM_USER | ARCH_MMU_FLAG_PERM_RO |
    ...
    EXPECT_TRUE(
        map_region_expect_failure(kaspace, ARCH_MMU_FLAG_PERM_USER | ARCH_MMU_FLAG_PERM_NO_EXECUTE,
                                  ERR_INVALID_ARGS),
        "6");
    EXPECT_TRUE(map_region_expect_failure(
                    kaspace,
                    ARCH_MMU_FLAG_PERM_USER | ARCH_MMU_FLAG_PERM_RO | ARCH_MMU_FLAG_PERM_NO_EXECUTE,
                    ERR_INVALID_ARGS),
                "7");
  }
