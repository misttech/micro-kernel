...
static int do_boot(lkb_t *lkb, size_t len, const char **result) {
  ...
  if (vmm_alloc_contiguous(vmm_get_kernel_aspace(), "lkboot_iobuf", len, &buf,
                           log2_uint(1024 * 1024), 0, ARCH_MMU_FLAG_UNCACHED) < 0) {
...
#error need better way
  ...
  thread_resume(
      thread_create("boot", &chainload_thread, &cl_args, DEFAULT_PRIORITY, DEFAULT_STACK_SIZE));
  ...
  *result = NULL;
  ...
  if (!strcmp(cmd, "flash") || !strcmp(cmd, "erase")) {
    ...
    *result = "no fpga";
    ...
    thread_resume(thread_create("reboot", &do_reboot, NULL, DEFAULT_PRIORITY, DEFAULT_STACK_SIZE));
  } else {
