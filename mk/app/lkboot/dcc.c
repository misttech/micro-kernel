...
static void send_pdcc_command(uint32_t opcode, uint32_t data) {
  ...
  word = PDCC_VALID | ((opcode & 0x7f) << PDCC_OPCODE_SHIFT) | (data & 0x00ffffff);
...
static void send_reset(void) { send_pdcc_command(PDCC_OP_RESET, 0); }

static void send_out_index_update(uint32_t index) {
  send_pdcc_command(PDCC_OP_UPDATE_OUT_INDEX, index);
}

static void send_buffer_consumed(void) { send_pdcc_command(PDCC_OP_CONSUMED_IN, 0); }
