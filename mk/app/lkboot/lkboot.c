...
int lkb_write(lkb_t *lkb, const void *_data, size_t len) {
  ...
  while (len > 0) {
    ...
    if (lkb_send(lkb, MSG_SEND_DATA, data, xfer))
      return -1;
    len -= xfer;
    data += xfer;
  }
...
int lkb_read(lkb_t *lkb, void *_data, size_t len) {
  ...
  if (lkb->state == STATE_OPEN) {
    if (lkb_send(lkb, MSG_GO_AHEAD, NULL, 0))
      return -1;
  }
  while (len > 0) {
    if (lkb->avail == 0) {
      msg_hdr_t hdr;
      if (lkb->read(lkb->cookie, &hdr, sizeof(hdr)))
        goto fail;
      if (hdr.opcode == MSG_END_DATA) {
        lkb->state = STATE_RESP;
        return -1;
      }
      if (hdr.opcode != MSG_SEND_DATA)
        goto fail;
      lkb->avail = ((size_t)hdr.length) + 1;
    }
    if (lkb->avail >= len) {
      if (lkb->read(lkb->cookie, data, len))
        goto fail;
      lkb->avail -= len;
      return 0;
    }
...
status_t lkboot_process_command(lkb_t *lkb) {
  ...
  if (lkb->read(lkb->cookie, &hdr, sizeof(hdr)))
    goto fail;
  if (hdr.opcode != MSG_CMD)
    goto fail;
  if (hdr.length > 127)
    goto fail;
  if (lkb->read(lkb->cookie, cmd, hdr.length))
    goto fail;
  cmd[hdr.length] = 0;
...
__WEAK bool platform_abort_autoboot(void) { return false; }
