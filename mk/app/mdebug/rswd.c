...
static u8 optable[16] = {
    [OP_RD | OP_DP | OP_X0] = RD_IDCODE, [OP_RD | OP_DP | OP_X4] = RD_DPCTRL,
    [OP_RD | OP_DP | OP_X8] = RD_RESEND, [OP_RD | OP_DP | OP_XC] = RD_BUFFER,
    [OP_WR | OP_DP | OP_X0] = WR_ABORT,  [OP_WR | OP_DP | OP_X4] = WR_DPCTRL,
    [OP_WR | OP_DP | OP_X8] = WR_SELECT, [OP_WR | OP_DP | OP_XC] = WR_BUFFER,
    [OP_RD | OP_AP | OP_X0] = RD_AP0,    [OP_RD | OP_AP | OP_X4] = RD_AP1,
    [OP_RD | OP_AP | OP_X8] = RD_AP2,    [OP_RD | OP_AP | OP_XC] = RD_AP3,
    [OP_WR | OP_AP | OP_X0] = WR_AP0,    [OP_WR | OP_AP | OP_X4] = WR_AP1,
    [OP_WR | OP_AP | OP_X8] = WR_AP2,    [OP_WR | OP_AP | OP_XC] = WR_AP3,
};

static const char *board_str = TARGET;
static const char *build_str = "fw v1.00 (" __DATE__ ", " __TIME__ ")";

static void _reboot(void) { platform_halt(HALT_ACTION_REBOOT, HALT_REASON_SW_RESET); }
...
void handle_rswd(void) {
  ...
  for (;;) {
    ...
    printx("[ recv %d words ]\n", rxc / 4);
    for (n = 0; n < (rxc / 4); n += 4) {
      printx("%08x %08x %08x %08x\n", rxbuffer[n], rxbuffer[n + 1], rxbuffer[n + 2],
             rxbuffer[n + 3]);
    }
#endif
