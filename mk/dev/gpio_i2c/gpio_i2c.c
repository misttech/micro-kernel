...
static status_t gpio_i2c_tx_common(gpio_i2c_state_t *s, uint8_t address, const uint8_t *reg,
...
static status_t gpio_i2c_rx_common(gpio_i2c_state_t *s, uint8_t address, const uint8_t *reg,
                                   void *buf, size_t cnt) {
  const gpio_i2c_info_t *i = s->info;
  status_t ret = ERR_I2C_NACK;
...
status_t gpio_i2c_write_reg_bytes(int bus, uint8_t address, uint8_t reg, const uint8_t *buf,
                                  size_t cnt) {
  gpio_i2c_state_t *s = gpio_i2c_states + bus;
  if (((unsigned)bus >= countof(gpio_i2c_states)) || !s->info)
    return ERR_NOT_FOUND;
...
status_t i2c_write_reg_bytes(int, uint8_t, uint8_t, const uint8_t *, size_t)
    __WEAK_ALIAS("gpio_i2c_write_reg_bytes");
status_t i2c_read_reg_bytes(int, uint8_t, uint8_t, uint8_t *, size_t)
    __WEAK_ALIAS("gpio_i2c_read_reg_bytes");
