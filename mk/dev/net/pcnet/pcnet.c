...
static status_t pcnet_init(struct device *dev) {
  ...
  if (pci_bus_mgr_find_device(&loc, config->device_id, config->vendor_id, config->index) !=
      _PCI_SUCCESSFUL) {
  ...
  thread_resume(
      thread_create("[pcnet bh]", pcnet_thread, dev, DEFAULT_PRIORITY, DEFAULT_STACK_SIZE));
...
static status_t pcnet_read_pci_config(struct device *dev, pci_location_t loc) {
  ...
  pci_write_config_half(
      loc, PCI_CONFIG_COMMAND,
