...
status_t e1000::init_device(pci_location_t loc, const e1000_id_features *id) {
  ...
  err = vmm_alloc_contiguous(vmm_get_kernel_aspace(), str, rxring_len * sizeof(rdesc),
                             (void **)&rxring_, 0, 0, ARCH_MMU_FLAG_UNCACHED);
  if (err != NO_ERROR) {
    return ERR_NOT_FOUND;
  }
  ...
  err = vmm_alloc_contiguous(vmm_get_kernel_aspace(), str, txring_len * sizeof(tdesc),
                             (void **)&txring_, 0, 0, ARCH_MMU_FLAG_UNCACHED);
  if (err != NO_ERROR) {
    return ERR_NOT_FOUND;
  }
  ...
  write_reg(e1000_reg::IMS,
            ims | (1 << 1) | (1 << 0));  // transmit queue empty, tx descriptor write back

  return NO_ERROR;
}

extern "C" status_t e1000_register_with_minip() {
