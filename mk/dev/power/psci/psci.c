...
uint32_t psci_version(void) { return psci_call(PSCI_VERSION, 0, 0, 0); }

int psci_cpu_on(int corenr, ulong entrypoint) {
  return psci_call(CPU_ON, corenr, entrypoint, corenr);
}

void psci_system_off(void) { psci_call(SYSTEM_OFF, 0, 0, 0); }

void psci_system_reset(void) { psci_call(SYSTEM_RESET, 0, 0, 0); }
