...
static enum handler_return virtio_gpu_irq_driver_callback(struct virtio_device *dev, uint ring,
...
static status_t send_command_response(struct virtio_gpu_dev *gdev, const void *cmd, size_t cmd_len,
                                      void **_res, size_t res_len) {
  DEBUG_ASSERT(gdev);
  DEBUG_ASSERT(cmd);
  DEBUG_ASSERT(_res);
...
static status_t allocate_2d_resource(struct virtio_gpu_dev *gdev, uint32_t *resource_id,
...
static status_t attach_backing(struct virtio_gpu_dev *gdev, uint32_t resource_id, void *ptr,
                               size_t buf_len) {
  status_t err;
...
static status_t set_scanout(struct virtio_gpu_dev *gdev, uint32_t scanout_id, uint32_t resource_id,
                            uint32_t width, uint32_t height) {
  status_t err;
...
static status_t flush_resource(struct virtio_gpu_dev *gdev, uint32_t resource_id, uint32_t width,
                               uint32_t height) {
  status_t err;
...
static status_t transfer_to_host_2d(struct virtio_gpu_dev *gdev, uint32_t resource_id,
                                    uint32_t width, uint32_t height) {
  status_t err;
...
status_t virtio_gpu_start(struct virtio_device *dev) {
  ...
  /* allocate a resource */
  err = allocate_2d_resource(gdev, &gdev->display_resource_id, gdev->pmode.r.width,
                             gdev->pmode.r.height);
  if (err < 0) {
  ...
  err = set_scanout(gdev, gdev->pmode_id, gdev->display_resource_id, gdev->pmode.r.width,
                    gdev->pmode.r.height);
  if (err < 0) {
  ...
  thread_t *t;
  t = thread_create("virtio gpu flusher", &virtio_gpu_flush_thread, (void *)gdev, HIGH_PRIORITY,
                    DEFAULT_STACK_SIZE);
...
static int virtio_gpu_flush_thread(void *arg) {
  ...
  for (;;) {
    ...
    /* transfer to host 2d */
    err = transfer_to_host_2d(gdev, gdev->display_resource_id, gdev->pmode.r.width,
                              gdev->pmode.r.height);
    if (err < 0) {
    ...
    /* resource flush */
    err =
        flush_resource(gdev, gdev->display_resource_id, gdev->pmode.r.width, gdev->pmode.r.height);
    if (err < 0) {
      LTRACEF("failed to flush resource\n");
      continue;
    }
