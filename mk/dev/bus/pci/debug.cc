      ...
      for (int dev = 0; dev < 32; dev++) {
        ...
        for (int fn = 0; fn < 8; fn++) {
          ...
          if (config.vendor_id != 0xffff) {
            printf(
                "%04x:%02x:%02x.%0x vendor_id=%04x device_id=%04x, header_type=%02x "
                "base_class=%02x, sub_class=%02x, interface=%02x, irq=%u\n",
                state.segment, state.bus, state.dev, state.fn, config.vendor_id, config.device_id,
                config.header_type, config.base_class, config.sub_class, config.program_interface,
...
static int pci_config(int argc, const cmd_args *argv, uint32_t flags) {
  ...
  if (!strcmp(argv[2].str, "dump")) {
    ...
    printf("command=%04x status=%04x pi=%02x sub cls=%02x base cls=%02x htype=%02x\n",
           config.command, config.status, config.program_interface, config.sub_class,
           config.base_class, config.header_type & PCI_HEADER_TYPE_MASK);
    ...
    if (header_type == PCI_HEADER_TYPE_STANDARD) {  // type 0
      for (i = 0; i < 6; i += 2) {
        printf("bar%d=%08x  bar%d=%08x\n", i, config.type0.base_addresses[i], i + 1,
               config.type0.base_addresses[i + 1]);
      }
    } else if (header_type == PCI_HEADER_TYPE_PCI_BRIDGE) {  // type 1
      printf("primary bus=%02x secondary=%02x subordinate=%02x\n", config.type1.primary_bus,
             config.type1.secondary_bus, config.type1.subordinate_bus);
      for (i = 0; i < 2; i += 2) {
        printf("bar%d=%08x  bar%d=%08x\n", i, config.type1.base_addresses[i], i + 1,
               config.type1.base_addresses[i + 1]);
      }
    ...
    hexdump8_ex(&config, sizeof(config), 0);
  } else if (!strcmp(argv[2].str, "rb") || !strcmp(argv[2].str, "rh") ||
             !strcmp(argv[2].str, "rw")) {
    if (argc != 7) {
      return -1;
    }
  ...
  } else if (!strcmp(argv[2].str, "mb") || !strcmp(argv[2].str, "mh") ||
             !strcmp(argv[2].str, "mw")) {
    if (argc != 8) {
      return -1;
    }
