...
status_t resource_allocator::set_range(const resource_range &range, bool prefetchable) {
  LTRACEF("range base %#llx size %#llx type %d prefetchable %d\n", range.base, range.size,
          range.type, prefetchable);
  type_to_range(range.type, prefetchable) = range;
  return NO_ERROR;
}

status_t resource_allocator::allocate_mmio(bool can_be_64bit, bool prefetchable, uint64_t size,
                                           uint8_t align, uint64_t *out) {
  ...
  for (;;) {
    ...
    LTRACEF(
        "range base %#llx size %#llx. request size %#llx align %u prefetchable %d can_be_64 %d\n",
