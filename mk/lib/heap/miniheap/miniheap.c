...
retry:
  ...
  struct free_heap_chunk *chunk;
  list_for_every_entry (&theheap.free_list, chunk, struct free_heap_chunk, node) {
    DEBUG_ASSERT((chunk->len % sizeof(void *)) ==
                 0);  // len should always be a multiple of pointer size
    ...
    if (chunk->len >= size) {
      ...
      if (chunk->len > size + sizeof(struct free_heap_chunk)) {
        ...
        struct free_heap_chunk *newchunk =
            heap_create_free_chunk((uint8_t *)ptr + size, chunk->len - size, true);
  ...
  retry:
    ...
    if (unlikely(end_page > start_page)) {
      ...
      if (start_page == start && end_page == end) {
        LTRACEF(
            "special case, after splitting off new chunk, free chunk completely covers page(s)\n");
        list_delete(&chunk->node);
        goto free_chunk;
      }
