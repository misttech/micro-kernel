 ...
 * find the properties.
 */
void read_address_size_cells(const void *fdt, int offset, int depth, uint32_t *address_cells,
                             uint32_t *size_cells) {
...
status_t read_base_len_pair(const uint8_t *prop_ptr, size_t prop_len, size_t address_cell_size,
                            size_t size_cell_size, uint64_t *base, uint64_t *len) {
  *base = 0;
  *len = 0;
...
status_t _fdt_walk(const void *fdt, callback cb) {
  ...
  for (;;) {
    ...
    LTRACEF_LEVEL(2, "name '%s', depth %d, address cells %u, size cells %u\n", name, state.depth,
                  state.address_cells[state.depth], state.size_cells[state.depth]);
  ...
  auto walker = [max_cpu_count, cpu, cpu_count](const fdt_walk_state &state, const char *name) {
    ...
    if (*cpu_count < max_cpu_count && strncmp(name, "cpu@", 4) == 0 && state.depth == 2) {
      ...
      if (prop_ptr) {
        ...
        auto isa_extensions_string =
            get_prop_string(state.fdt, state.offset, "riscv,isa-extensions");
...
status_t fdt_walk_find_memory(const void *fdt, struct fdt_walk_memory_region *memory,
                              size_t *mem_count, struct fdt_walk_memory_region *reserved_memory,
                              size_t *reserved_mem_count) {
    ...
    if (memory && *mem_count < max_memory_index) {
      if (strncmp(name, "memory@", 7) == 0 && state.depth == 1) {
        ...
        const uint8_t *prop_ptr =
            (const uint8_t *)fdt_getprop(state.fdt, state.offset, "reg", &lenp);
        if (prop_ptr) {
          LTRACEF_LEVEL(2, "found '%s' reg prop len %d, ac %u, sc %u\n", name, lenp,
                        state.curr_address_cell(), state.curr_size_cell());
          /* we're looking at a memory descriptor */
          uint64_t base;
          uint64_t len;
          err = read_base_len_pair(prop_ptr, lenp, state.curr_address_cell(),
                                   state.curr_size_cell(), &base, &len);
          if (err != NO_ERROR) {
  ...
  *count = 0;
  auto walker = [info, info_len, &count](const fdt_walk_state &state, const char *name) {
    ...
    if (*count < info_len && (strncmp(name, "pcie@", 5) == 0 || strncmp(name, "pci@", 4) == 0)) {
      ...
      const uint8_t *prop_ptr =
          (const uint8_t *)fdt_getprop(state.fdt, state.offset, "status", &lenp);
      if (prop_ptr) {
