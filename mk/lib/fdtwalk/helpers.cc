...
status_t fdtwalk_setup_memory(const void *fdt, paddr_t fdt_phys, paddr_t default_mem_base,
  ...
  for (size_t i = 0; i < mem_count; i++) {
    ...
    dprintf(INFO, "FDT: found memory bank range [%#llx, %#llx] (length %#llx)\n", mem[i].base,
            mem[i].base + mem[i].len - 1, mem[i].len);
  ...
  for (size_t i = 0; i < reserved_mem_count; i++) {
    dprintf(INFO, "FDT: reserving memory range [%#llx, %#llx]\n", reserved_mem[i].base,
            reserved_mem[i].base + reserved_mem[i].len - 1);
  ...
  if (err == NO_ERROR) {
    for (size_t i = 0; i < count; i++) {
      LTRACEF("ecam base %#" PRIx64 ", len %#" PRIx64 ", bus_start %hhu, bus_end %hhu\n",
              pcie_info[i].ecam_base, pcie_info[i].ecam_len, pcie_info[i].bus_start,
              pcie_info[i].bus_end);
      ...
      if (pcie_info[i].ecam_len > 0) {
        ...
        err = pci_init_ecam(pcie_info[i].ecam_base, pcie_info[i].ecam_len, pcie_info[i].bus_start,
                            pcie_info[i].bus_end);
        if (err == NO_ERROR) {
          ...
          if (pcie_info[i].mmio_len > 0) {
            pci_bus_mgr_add_resource(PCI_RESOURCE_MMIO_RANGE, pcie_info[i].mmio_base,
                                     pcie_info[i].mmio_len);
          }
          if (sizeof(void *) >= 8) {
            if (pcie_info[i].mmio64_len > 0) {
              pci_bus_mgr_add_resource(PCI_RESOURCE_MMIO64_RANGE, pcie_info[i].mmio64_base,
