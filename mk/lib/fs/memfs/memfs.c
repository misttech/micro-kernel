...
static status_t memfs_create(fscookie *cookie, const char *name, filecookie **fcookie,
                             uint64_t len) {
  status_t err;
...
static status_t memfs_readdir(dircookie *dcookie, struct dirent *ent) {
  ...
  if (dcookie->next_file) {
    ...
    dcookie->next_file =
        list_next_type(&dcookie->fs->files, &dcookie->next_file->node, memfs_file_t, node);
    err = NO_ERROR;
  } else {
    err = ERR_NOT_FOUND;
  }
