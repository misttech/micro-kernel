...
int ext2_put_block(ext2_t *ext2, blocknum_t bnum) { return bcache_put_block(ext2->cache, bnum); }

static int ext2_calculate_block_pointer_pos(ext2_t *ext2, blocknum_t block_to_find, uint32_t *level,
                                            uint32_t pos[]) {
...
// This function returns a pointer to the cache block that corresponds to the indirect block
// pointer.
static int ext2_get_indirect_block_pointer_cache_block(ext2_t *ext2, struct ext2_inode *inode,
                                                       blocknum_t **cache_block, uint32_t level,
                                                       uint32_t pos[], uint *block_loaded) {
...
static blocknum_t file_block_to_fs_block(ext2_t *ext2, struct ext2_inode *inode, uint fileblock) {
  ...
  if (level == 0) {
    ...
    /* at least one level of indirection, get a pointer to the final indirect block table and
     * dereference it */
    blocknum_t *ind_table;
    blocknum_t phys_block;
    err = ext2_get_indirect_block_pointer_cache_block(ext2, inode, &ind_table, level, pos,
                                                      &phys_block);
    if (err < 0)
      return 0;
...
ssize_t ext2_read_inode(ext2_t *ext2, struct ext2_inode *inode, void *_buf, off_t offset,
                        size_t len) {
  int err = 0;
  size_t bytes_read = 0;
  uint8_t *buf = _buf;
