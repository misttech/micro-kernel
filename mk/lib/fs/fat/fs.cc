...
status_t fat_fs::mount(bdev_t *dev, fscookie **cookie) {
  ...
  if ((info->bytes_per_sector != 0x200) && (info->bytes_per_sector != 0x400) &&
      (info->bytes_per_sector != 0x800)) {
  ...
  info->root_dir_sectors =
      ((info->root_entries * 32) + (info->bytes_per_sector - 1)) / info->bytes_per_sector;
  ...
  // first data sector is just after the root dir (in fat12/16) which is just after the FAT(s),
  // which is just after the reserved sectors
  info->data_start_sector =
      info->reserved_sectors + (info->fat_count * info->sectors_per_fat) + info->root_dir_sectors;
  ...
  if (info->fat_bits == 32) {
    ...
    // In FAT32, root directory acts much like a file and occupies a cluster chain starting
    // generally at cluster 2.
    ...
    // On a FAT 12 or FAT 16 volumes the root directory is at a fixed position immediately after the
