...
static status_t cursor_init(cursor_t *cursor, spifs_t *spifs, int32_t direction, uint32_t page_id,
                            uint32_t entry_length) {
...
static uint32_t find_open_run(spifs_t *spifs, uint32_t requested_length) {
  ...
  list_for_every_entry (&spifs->files, file, spifs_file_t, node) {
    ...
    uint32_t file_page_length = divpow2(file->metadata.capacity, page_size_shift);
    ...
    spifs_file_t *next = list_next_type(&spifs->files, &file->node, spifs_file_t, node);
...
static bool consistency_check(spifs_t *spifs) {
  ...
  list_for_every_entry (&spifs->files, file, spifs_file_t, node) {
    // Number of pages that this file occupies
    uint32_t file_page_length = file->metadata.capacity / file->fs_handle->page_size;
...
static status_t spifs_commit_toc(spifs_t *spifs) {
  ...
  toc_position_t target_toc = spifs->toc_position == FRONT_TOC ? BACK_TOC : FRONT_TOC;
  ...
  // at this point.
#if LK_DEBUGLEVEL > 1
  uint8_t *expected_cursor_location = (spifs->page + spifs->page_size) - SPIFS_ENTRY_LENGTH;
...
static status_t spifs_read_page(spifs_t *spifs, uint32_t page_addr) {
  ...
  ssize_t bytes = bio_read_block(spifs->dev, spifs->page, block_addr, spifs->blocks_per_page);
...
static status_t spifs_write_page(spifs_t *spifs, uint32_t page_addr) {
  ...
  ssize_t bytes = bio_write_block(spifs->dev, spifs->page, block_addr, spifs->blocks_per_page);
...
static uint32_t get_toc_generation(spifs_t *spifs, toc_position_t toc_pos) {
  ...
  cursor_t cursor;
  if (cursor_init(&cursor, spifs, toc_pos, toc_page, SPIFS_ENTRY_LENGTH) != NO_ERROR) {
    return CORRUPT_TOC;
  }
...
static status_t spifs_mount(bdev_t *dev, fscookie **cookie) {
  ...
  spifs->toc_position = f_toc_generation > b_toc_generation ? FRONT_TOC : BACK_TOC;
  ...
  cursor_t cursor;
  status = cursor_init(&cursor, spifs, spifs->toc_position, toc_page_addr, SPIFS_ENTRY_LENGTH);
  if (status != NO_ERROR)
    goto err;
...
static status_t spifs_create(fscookie *cookie, const char *name, filecookie **fcookie,
                             uint64_t len) {
  ...
  if (bio_erase(spifs->dev, open_run * spifs->page_size, capacity) != (ssize_t)capacity) {
    free(file);
...
static status_t spifs_remove(fscookie *cookie, const char *name) {
  ...
  list_for_every_entry (&spifs->dcookies, dcookie, dircookie, node) {
    if (dcookie->next_file == file) {
      dcookie->next_file =
          list_next_type(&dcookie->fs->files, &dcookie->next_file->node, spifs_file_t, node);
    }
  }
...
static ssize_t spifs_write(filecookie *fcookie, const void *buf, off_t off, size_t size) {
  ...
  uint32_t start_addr = off + (file->metadata.page_idx * spifs->page_size);
...
static status_t spifs_opendir(fscookie *cookie, const char *name, dircookie **dcookie) {
  ...
  spifs_file_t *front_toc_file = list_peek_head_type(&spifs->files, spifs_file_t, node);
  dir->next_file = list_next_type(&spifs->files, &front_toc_file->node, spifs_file_t, node);
...
static status_t spifs_readdir(dircookie *dcookie, struct dirent *ent) {
  ...
  spifs_file_t *back_toc_file = list_peek_tail_type(&dcookie->fs->files, spifs_file_t, node);
