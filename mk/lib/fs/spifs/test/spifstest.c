...
static bool test_setup(const char *dev_name, uint32_t toc_pages) {
  ...
  if (res != NO_ERROR) {
    printf(
        "fs_mount failed path = %s, fs name = %s, dev name = %s,"
        " retcode = %d\n",
...
static bool test_write_read_normal(const char *dev_name) {
  ...
  filehandle *handle;
  status_t status = fs_create_file(TEST_FILE_PATH, &handle, sizeof(test_message));
  if (status != NO_ERROR) {
    return false;
  }
...
static bool test_full_toc(const char *dev_name) {
  ...
  for (size_t i = 0; i < stat.free_inodes; i++) {
    ...
    status_t status = fs_create_file(test_file_name, &handle, 1);
...
static bool test_rm_reclaim(const char *dev_name) {
  ...
  for (size_t i = 0; i < n_files; i++) {
    ...
    status_t status = fs_create_file(test_file_name, &handle, file_size);
...
static bool test_write_with_offset(const char *dev_name) {
  ...
  for (size_t i = 0; i < repeats; i++) {
    success &= (memcmp(test_message, test_buf + i * msg_len, msg_len) == 0);
  }
  return success;
}
...
static bool test_truncate_file(const char *dev_name) {
  filehandle *handle;
  status_t status = fs_create_file(TEST_FILE_PATH, &handle, 1024);
  if (status != NO_ERROR) {
    return false;
  }
...
static int spifs_bench(int argc, const console_cmd_args *argv) {
  ...
  for (size_t file_size = min_bench_bytes; file_size <= max_bench_bytes; file_size *= 2) {
    ...
    if (st != NO_ERROR) {
      printf(
          "SPIFS Benchmark Failed to create a %zu byte file at %s. "
          "Reason = %d.\n",
    ...
    start = current_time_hires();
    do {
      size_t write_length = MIN(test_buffer_length, file_size - offset);
    ...
    if (n_bytes < 0) {
      printf(
          "SPIFS Benchmark Failed to write to file at %s. "
          "Reason = %ld.\n",
    ...
    start = current_time_hires();
    do {
      n_bytes = fs_read_file(handle, test_buffer, offset, test_buffer_length);
      offset += n_bytes;
    } while (n_bytes > 0);
    ...
    if (n_bytes < 0) {
      printf(
          "SPIFS Benchmark Failed to read from file at %s. "
          "Reason = %ld.\n",
          test_file_path, n_bytes);
      retcode = -1;
      fs_close_file(handle);
    ...
    if (st != NO_ERROR) {
      printf(
          "SPIFS Benchmark Failed to close file at %s. "
          "Reason = %d.\n",
          test_file_path, st);
      retcode = -1;
      goto finish;
    ...
    if (st != NO_ERROR) {
      printf(
          "SPIFS Benchmark Failed to remove file at %s. "
          "Reason = %d.\n",
          test_file_path, st);
      retcode = -1;
      goto finish;
