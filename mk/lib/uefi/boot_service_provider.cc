...
void *alloc_page(size_t size, size_t align_log2) {
  auto aspace = set_boot_aspace();
  void *vptr{};
  status_t err = vmm_alloc_contiguous(aspace, "uefi_program", size, &vptr, align_log2, 0, 0);
  if (err) {
...
void *alloc_page(void *addr, size_t size, size_t align_log2) {
  ...
  auto err = vmm_alloc_contiguous(set_boot_aspace(), "uefi_program", size, &addr, align_log2,
...
bool guid_eq(const EfiGuid *a, const EfiGuid *b) { return memcmp(a, b, sizeof(*a)) == 0; }
...
void fill_memory_map_entry(vmm_aspace_t *aspace, EfiMemoryDescriptor *entry,
  ...
  paddr_t pa{};
  uint flags{};
  status_t err = arch_mmu_query(&aspace->arch_aspace, region->base, &pa, &flags);
  if (err >= 0) {
...
EfiStatus get_physical_memory_map(size_t *memory_map_size, EfiMemoryDescriptor *memory_map,
...
EfiStatus get_memory_map(size_t *memory_map_size, EfiMemoryDescriptor *memory_map, size_t *map_key,
...
EfiStatus register_protocol_notify(const EfiGuid *protocol, EfiEvent event, void **registration) {
...
EfiStatus locate_protocol(const EfiGuid *protocol, void *registration, void **intf) {
  ...
  printf("%s(%x %x %x %llx) is unsupported\n", __FUNCTION__, protocol->data1, protocol->data2,
         protocol->data3, *reinterpret_cast<const uint64_t *>(&protocol->data4));
  return UNSUPPORTED;
}
...
EfiStatus load_image(bool boot_policy, EfiHandle parent_image_handle, EfiDevicePathProtocol *path,
                     void *src, size_t src_size, EfiHandle *image_handle) {
  printf("%s is unsupported\n", __FUNCTION__);
  return UNSUPPORTED;
}
...
EfiStatus locate_device_path(const EfiGuid *protocol, EfiDevicePathProtocol **path,
                             EfiHandle *device) {
  if (memcmp(protocol, &EFI_LOAD_FILE2_PROTOCOL_GUID, sizeof(EFI_LOAD_FILE2_PROTOCOL_GUID)) == 0) {
    return NOT_FOUND;
  }
  printf("%s is unsupported\n", __FUNCTION__);
  return UNSUPPORTED;
...
void copy_mem(void *dest, const void *src, size_t len) { memcpy(dest, src, len); }
...
EfiStatus read_blocks(EfiBlockIoProtocol *self, uint32_t media_id, uint64_t lba, size_t buffer_size,
                      void *buffer) {
  ...
  const size_t bytes_read = call_with_stack(interface->io_stack, bio_read_block, dev, buffer, lba,
...
EfiStatus open_block_device(EfiHandle handle, void **intf) {
  ...
  if (io_stack == nullptr) {
    vmm_alloc(vmm_get_kernel_aspace(), "uefi_io_stack", kIoStackSize, &io_stack, PAGE_SIZE_SHIFT, 0,
              0);
  }
  printf("%s(%p)\n", __FUNCTION__, handle);
...
EFI_STATUS efi_dt_fixup(struct EfiDtFixupProtocol *self, void *fdt, size_t *buffer_size,
                        uint32_t flags) {
  ...
  static constexpr auto &&to_add = "console=ttyAMA0 earlycon=pl011,mmio32,0x9000000 ";
...
EfiStatus fixup_bootconfig(struct GblEfiOsConfigurationProtocol *self, const char *bootconfig,
  ...
  printf("%s(%p, %s, %lu, %lu)\n", __FUNCTION__, self, bootconfig, size, *fixup_buffer_size);
...
EfiStatus select_device_trees(struct GblEfiOsConfigurationProtocol *self,
  ...
  printf("%s(%p, %p %lu)\n", __FUNCTION__, self, device_trees, num_device_trees);
  return UNSUPPORTED;
}
...
EfiStatus open_protocol(EfiHandle handle, const EfiGuid *protocol, void **intf,
  ...
  if (guid_eq(protocol, LOADED_IMAGE_PROTOCOL_GUID)) {
    ...
    *intf = interface;
    printf(
        "%s(LOADED_IMAGE_PROTOCOL_GUID, handle=%p, agent_handle=%p, "
    ...
    printf(
        "%s(EFI_BLOCK_IO_PROTOCOL_GUID, handle=%p, agent_handle=%p, "
        "controller_handle=%p, attr=0x%x)\n",
        __FUNCTION__, handle, agent_handle, controller_handle, attr);
    return open_block_device(handle, intf);
  } else if (guid_eq(protocol, EFI_BLOCK_IO2_PROTOCOL_GUID)) {
    printf(
        "%s(EFI_BLOCK_IO2_PROTOCOL_GUID, handle=%p, agent_handle=%p, "
        "controller_handle=%p, attr=0x%x)\n",
        __FUNCTION__, handle, agent_handle, controller_handle, attr);
    return UNSUPPORTED;
  } else if (guid_eq(protocol, EFI_DT_FIXUP_PROTOCOL_GUID)) {
    printf(
        "%s(EFI_DT_FIXUP_PROTOCOL_GUID, handle=%p, agent_handle=%p, "
        "controller_handle=%p, attr=0x%x)\n",
        __FUNCTION__, handle, agent_handle, controller_handle, attr);
    if (intf != nullptr) {
    ...
    printf(
        "%s(EFI_GBL_OS_CONFIGURATION_PROTOCOL_GUID, handle=%p, "
        "agent_handle=%p, "
        "controller_handle=%p, attr=0x%x)\n",
        __FUNCTION__, handle, agent_handle, controller_handle, attr);
    GblEfiOsConfigurationProtocol *config = nullptr;
    allocate_pool(BOOT_SERVICES_DATA, sizeof(*config), reinterpret_cast<void **>(&config));
    if (config == nullptr) {
      return OUT_OF_RESOURCES;
    }
  ...
  printf("%s is unsupported 0x%x 0x%x 0x%x 0x%llx\n", __FUNCTION__, protocol->data1,
         protocol->data2, protocol->data3, *(uint64_t *)&protocol->data4);
  return UNSUPPORTED;
}
...
EfiStatus list_block_devices(size_t *num_handles, EfiHandle **buf) {
  ...
  auto devices =
      reinterpret_cast<char **>(mspace_malloc(get_mspace(), sizeof(char *) * device_count));
  size_t i = 0;
...
EfiStatus locate_handle_buffer(EfiLocateHandleSearchType search_type, const EfiGuid *protocol,
  ...
  if (guid_eq(protocol, EFI_BLOCK_IO_PROTOCOL_GUID)) {
    ...
    if (buf != nullptr) {
      *buf = reinterpret_cast<EfiHandle *>(mspace_malloc(get_mspace(), sizeof(buf)));
    }
    return SUCCESS;
  } else if (guid_eq(protocol, EFI_DT_FIXUP_PROTOCOL_GUID)) {
  ...
  printf("%s(0x%x, (0x%x 0x%x 0x%x 0x%llx), search_key=%p)\n", __FUNCTION__, search_type,
         protocol->data1, protocol->data2, protocol->data3, *(uint64_t *)&protocol->data4,
         search_key);
  return UNSUPPORTED;
}

EfiStatus wait_for_event(size_t num_events, EfiEvent *event, size_t *index) { return UNSUPPORTED; }
...
void setup_boot_service_table(EfiBootService *service) {
  ...
  service->uninstall_multiple_protocol_interfaces = uninstall_multiple_protocol_interfaces;
