...
static enum handler_return hw_watchdog_timer_callback(struct timer *timer, lk_time_t now,
                                                      void *arg) {
...
void watchdog_hw_set_enabled(bool enabled) {
  ...
  if (enabled)
    timer_set_periodic(&hw_watchdog_timer, hw_watchdog_pet_timeout, hw_watchdog_timer_callback,
                       NULL);
  else
