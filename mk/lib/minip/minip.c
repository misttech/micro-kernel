...
static void check_and_set_configured(void) {
  if (minip_ip == IPV4_NONE)
    return;
  if (minip_netmask == IPV4_NONE)
    return;
  if (minip_broadcast == IPV4_BCAST)
    return;
  // minip_gateway doesn't have to be set
  if (minip_mac[0] == 0xcc && minip_mac[1] == 0xcc && minip_mac[2] == 0xcc &&
      minip_mac[3] == 0xcc && minip_mac[4] == 0xcc && minip_mac[5] == 0xcc)
    return;
...
const char *minip_get_hostname(void) { return minip_hostname; }

static void compute_broadcast_address(void) {
  minip_broadcast = (minip_ip & minip_netmask) | (IPV4_BCAST & ~minip_netmask);
}

void minip_get_macaddr(uint8_t *addr) { mac_addr_copy(addr, minip_mac); }

uint32_t minip_get_ipaddr(void) { return minip_ip; }
...
uint32_t minip_get_broadcast(void) { return minip_broadcast; }

uint32_t minip_get_netmask(void) { return minip_netmask; }
...
uint32_t minip_get_gateway(void) { return minip_gateway; }
...
bool minip_is_configured(void) { return minip_configured; }
...
static void dump_ipv4_packet(const struct ipv4_hdr *ip) {
  ...
  printf(" hlen 0x%x, prot 0x%x, cksum 0x%x, len 0x%x, ident 0x%x, frag offset 0x%x\n",
         (ip->ver_ihl & 0xf) * 4, ip->proto, ntohs(ip->chksum), ntohs(ip->len), ntohs(ip->id),
         ntohs(ip->flags_frags) & 0x1fff);
}
...
void minip_rx_driver_callback(pktbuf_t *p) {
  ...
  if (memcmp(eth->dst_mac, minip_mac, 6) != 0 && memcmp(eth->dst_mac, broadcast_mac, 6) != 0) {
    /* not for us */
    return;
  }
