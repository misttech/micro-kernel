...
void dhcp::udp_callback(void *data, size_t sz, uint32_t srcip, uint16_t srcport) {
  ...
  if (sz < sizeof(dhcp_msg_t))
    return;

  uint8_t mac[6];
  minip_get_macaddr(mac);
  if (memcmp(msg->chaddr, mac, 6))
    return;
  ...
  while (sz >= 2) {
    ...
    switch (opt[0]) {
      ...
      case OPT_NET_MASK:
        if (opt[1] == 4)
          memcpy(&netmask, opt + 2, 4);
        break;
      case OPT_ROUTERS:
        if (opt[1] >= 4)
          memcpy(&gateway, opt + 2, 4);
        break;
      case OPT_DNS:
        if (opt[1] >= 4)
          memcpy(&dns, opt + 2, 4);
  ...
  printf("\n\t");
  if (server)
    printip_named("server", server);
  if (netmask)
    printip_named(" netmask", netmask);
  if (gateway)
    printip_named(" gateway", gateway);
  if (dns)
    printip_named(" dns", dns);
  printf("\n");
#endif
