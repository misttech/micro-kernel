...
void bio_initialize_bdev(bdev_t *dev, const char *name, size_t block_size, bnum_t block_count,
                         size_t geometry_count, const bio_erase_geometry_info_t *geometry,
...
/* subdevice support */
status_t bio_publish_subdevice(const char *parent_dev, const char *subdev, bnum_t startblock,
                               bnum_t block_count);
...
/* utility routine */
static inline bool bio_does_overlap(uint64_t start1, uint64_t len1, uint64_t start2,
                                    uint64_t len2) {
...
static inline bool bio_contains_range(uint64_t container_start, uint64_t container_len,
  ...
  return ((container_start <= contained_start) && (container_end >= contained_end));
}
...
enum bio_ioctl_num {
  ...
  BIO_IOCTL_GET_MAP_ADDR, /* if supported, request a pointer to the memory map without putting the
                             device into linear mode */
