...
static ssize_t bio_default_write_block(struct bdev *dev, const void *buf, bnum_t block,
                                       uint count) {
  return ERR_NOT_SUPPORTED;
}
...
void bio_dump_devices(void) {
  ...
  mutex_acquire(&bdevs.lock);
  list_for_every_entry (&bdevs.list, entry, bdev_t, node) {
    printf("\t%s, size %lld, bsize %zd, ref %d", entry->name, entry->total_size, entry->block_size,
           entry->ref);
    ...
    if (!entry->geometry_count || !entry->geometry) {
      ...
      for (size_t i = 0; i < entry->geometry_count; ++i) {
        ...
        printf("\n\t\terase_region[%zu] : start %lld size %lld erase size %zu", i, geo->start,
               geo->size, geo->erase_size);
      }
    }
