...
#if LK_DEBUGLEVEL > 0
static int cmd_bio(int argc, const cmd_args *argv, uint32_t flags);
...
static status_t memory_mapped_test(bdev_t *device) {
  ...
  if (!test_buffer) {
    printf(
        "Could not allocate %zu bytes for a temporary buffer. "
        "Aborting.\n",
        device->block_size);
  ...
  if (!reference_buffer) {
    printf(
        "Could not allocate %zu bytes for a temporary reference "
        "buffer. Aborting.\n",
        device->block_size);
    free(test_buffer);
    return ERR_NO_MEMORY;
  ...
  if (err < (ssize_t)device->block_size) {
    printf(
        "Expected to erase at least %zu bytes but only erased %ld. "
  ...
  if (err != (ssize_t)device->block_size) {
    printf(
        "Error while writing test pattern to device. Expected to write "
  ...
  for (uint i = 0; i < device->block_size; i++) {
    if (*testptr != *devaddr) {
      printf(
          "Data mismatch at position %d. Expected %d got %d. "
          "Aborting.\n",
          i, *testptr, *devaddr);
      goto finish;
    }
