...
#define PTABLE_HEADER_NUM_ENTRIES(header) \
  (((header).total_length - sizeof(struct ptable_header)) / sizeof(struct ptable_entry))
#define BAIL(__err) \
  do {              \
    err = __err;    \
    goto bailout;   \
...
static status_t ptable_write(void) {
  ...
  size_t count = 0;
  struct list_node *node;
  list_for_every(&ptable.list, node) { count++; }
  LTRACEF("%u entries\n", count);
...
static status_t ptable_publish(const struct ptable_entry *entry) {
  ...
  if ((entry->offset & block_mask) || (entry->length & block_mask)) {
    LTRACEF(
        "Entry in parition (\"%s\") is misaligned "
  ...
  err =
      bio_publish_subdevice(ptable.bdev->name, part_name, entry->offset >> ptable.bdev->block_shift,
...
static off_t ptable_adjust_request_for_erase_geometry(uint64_t region_start, uint64_t region_len,
                                                      uint64_t *plength, bool alloc_end) {
  DEBUG_ASSERT(plength && ptable.bdev);

  LTRACEF("[0x%llx, 0x%llx) len 0x%llx%s\n", region_start, region_start + region_len, *plength,
          alloc_end ? " (alloc end)" : "");
  ...
  for (size_t i = 0; i < ptable.bdev->geometry_count; ++i) {
    ...
    LTRACEF("Considering erase region [0x%llx, 0x%llx) (erase size 0x%zx)\n", geo->start,
            geo->start + geo->size, geo->erase_size);
...
static off_t ptable_allocate(uint64_t *plength, uint flags) {
  ...
  if (list_is_empty(&ptable.list)) {
     ...
     */
    offset =
        ptable_adjust_request_for_erase_geometry(0, ptable.bdev->total_size, &length, alloc_end);
    goto done;
  }
  ...
  list_for_every_entry (&ptable.list, mentry, struct ptable_mem_entry, node) {
    ...
    LTRACEF("Considering region [0x%llx, 0x%llx) between \"%s\" and \"%s\"\n", region_start,
            region_start + region_len, lastentry ? (char *)lastentry->name : "<device start>",
            entry->name);
    lastentry = entry;
    ...
    test_len = length;
    test_offset =
        ptable_adjust_request_for_erase_geometry(region_start, region_len, &test_len, alloc_end);
  ...
  if (region_len) {
    LTRACEF("Considering region [0x%llx, 0x%llx) between \"%s\" and \"%s\"\n", region_start,
            region_start + region_len, lastentry->name, "<device end>");
    test_len = length;
    test_offset =
        ptable_adjust_request_for_erase_geometry(region_start, region_len, &test_len, alloc_end);
    if (test_offset >= 0) {
...
static status_t ptable_allocate_at(off_t _offset, uint64_t *plength) {
  ...
  *plength = (*plength + block_mask) & ~block_mask;
   ...
   */
  uint64_t new_offset = ptable_adjust_request_for_erase_geometry(
      offset, ptable.bdev->total_size - offset, plength, false);
...
bool ptable_found_valid(void) { return (NULL != ptable.bdev); }

bdev_t *ptable_get_device(void) { return ptable.bdev; }
...
static int cmd_ptable(int argc, const console_cmd_args *argv) {
  ...
  } else if (!strcmp(argv[1].str, "alloc") || !strcmp(argv[1].str, "allocend")) {
    if (argc < 3)
      goto notenoughargs;
