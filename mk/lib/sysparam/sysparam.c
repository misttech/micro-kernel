...
static void sysparam_init(uint level) { list_initialize(&params.list); }
...
static struct sysparam *sysparam_create(const char *name, size_t namelen, const void *data,
...
static struct sysparam *sysparam_read_phys(const struct sysparam_phys *sp) {
  return sysparam_create((const char *)sp->namedata, sp->namelen,
                         sp->namedata + ROUNDUP(sp->namelen, 4), sp->datalen, sp->flags);
}
...
void sysparam_dump(bool show_all) {
  ...
  struct sysparam *param;
  list_for_every_entry (&params.list, param, struct sysparam, node) {
    printf("________%c %-16s : ", (param->flags & SYSPARAM_FLAG_LOCK) ? 'L' : '_', param->name);
