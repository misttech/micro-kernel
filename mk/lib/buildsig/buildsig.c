...
status_t buildsig_search(const void *_ptr, size_t search_len, size_t max_len,
                         const lk_version_t **version) {
...
#define VALIDSTR(str) (((size_t)(str) >= (size_t)ptr) && (((size_t)(str) - (size_t)ptr) < max_len))
...
static int cmd_buildsig(int argc, const console_cmd_args *argv) {
  ...
  if (!strcmp(argv[1].str, "dump")) {
    ...
    printf("\tarch: %s\n\tplatform: %s\n\ttarget: %s\n\tproject: %s\n\tbuildid: %s\n", v->arch,
           v->platform, v->target, v->project, v->buildid);
  } else {
    goto usage;
  }
