...
void arch_init(void) {
  ...
  secondaries_to_init =
      SMP_MAX_CPUS - 1; /* TODO: get count from somewhere else, or add cpus as they boot */
...
void arch_idle(void) { __asm__ volatile("wfi"); }
...
void arch_enter_uspace(vaddr_t entry_point, vaddr_t user_stack_top) {
  ...
  asm volatile(
      ...
      "eret;"
      :
      : [ustack] "r"(user_stack_top), [kstack] "r"(kernel_stack_top), [entry] "r"(entry_point),
        [spsr] "r"(spsr)
      : "memory");
  __UNREACHABLE;
}
