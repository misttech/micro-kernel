...
static const struct fault_status_map fsc_map[] = {
    ...
    {.fsc = 0b010000, .fault_msg = "Synchronous External abort, not on translation table walk"},
...
void arm64_sync_exception(struct arm64_iframe_long *iframe) {
  ...
  switch (ec) {
    ...
    case 0b100101: { /* data abort from same level */
      for (fault_handler = __fault_handler_table_start; fault_handler < __fault_handler_table_end;
           fault_handler++) {
