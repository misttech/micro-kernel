...
#define KERNEL_ASPACE_BITS                                               \
  KERNEL_BASE_BITS /* KERNEL_BASE should not be below KERNEL_ASPACE_BASE \
                    */
#endif
...
#define MMU_TG0(page_size_shift) \
  ((((page_size_shift == 14) & 1) << 1) | ((page_size_shift == 16) & 1))
...
#define MMU_PTE_KERNEL_RO_FLAGS                                           \
  (MMU_PTE_ATTR_UXN | MMU_PTE_ATTR_AF | MMU_PTE_ATTR_SH_INNER_SHAREABLE | \
