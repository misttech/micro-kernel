...
static const char *ide_type_str[] = {
    "None", "Unknown", "Floppy", "IDE CDROM", "SCSI CDROM", "IDE Disk", "SCSI Disk",
};

static const char *ide_error_str[] = {"Unknown error",
...
static status_t ide_init(struct device *dev) {
  ...
  if (config->legacy_index == 0x80 || config->legacy_index == 0x81) {
    ...
    status_t err =
        pci_bus_mgr_find_device_by_class(&loc, PCI_CLASS_MASS_STORAGE, PCI_SUBCLASS_IDE, 0x80, 0);
    if (err != NO_ERROR) {
...
static status_t ide_detect_ata(struct device *dev, int index) {
  ...
  dprintf(INFO, "ide: Disk %d supports %u sectors for a total of %u bytes\n", index,
          state->drive[index].sectors, state->drive[index].sectors * 512);
...
static void ide_lba_setup(struct device *dev, uint32_t addr, int drive) {
  ide_write_reg8(dev, IDE_REG_DRIVE_HEAD,
                 0xe0 | ((drive & 0x00000001) << 4) | ((addr >> 24) & 0xf));
