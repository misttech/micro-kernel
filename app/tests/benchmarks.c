...
#define bench_cset(type)                                                                                                    \
  __NO_INLINE static void bench_cset_##type(void) {                                                                         \
    ...
    double bytes_cycle = total_bytes / (double)count;                                                                       \
    printf(                                                                                                                 \
        "took %lu cycles to manually clear a buffer using wordsize %zu of size %zu %u times (%zu bytes), %f bytes/cycle\n", \
        count, sizeof(*buf), BUFSIZE, ITER, total_bytes, bytes_cycle);                                                      \
                                                                                                                            \
    free(buf);                                                                                                              \
  }

bench_cset(uint8_t) bench_cset(uint16_t) bench_cset(uint32_t) bench_cset(uint64_t)
  ...
  double bytes_cycle = total_bytes / (double)count;
  printf(
      "took %lu cycles to manually clear a buffer of size %zu %d times 8 words at a time (%zu bytes), %f bytes/cycle\n",
      count, BUFSIZE, ITER, total_bytes, bytes_cycle);
...
__NO_INLINE static void bench_memcpy(void) {
  ...
  double bytes_cycle = total_bytes / (double)count;
  printf(
      "took %lu cycles to memcpy a buffer of size %zu %d times (%zu source bytes), %f source bytes/cycle\n",
...
__NO_INLINE static void arm_bench_cset_stm(void) {
  ...
  for (uint i = 0; i < ITER; i++) {
    for (uint j = 0; j < BUFSIZE / sizeof(*buf) / 8; j++) {
      __asm__ volatile("stm    %0, {r0-r7};" ::"r"(&buf[j * 8]));
    }
  }
  count = arch_cycle_count() - count;
  ...
  printf(
      "took %lu cycles to manually clear a buffer of size %zu %d times 8 words at a time using stm (%zu bytes), %f bytes/cycle\n",
      count, BUFSIZE, ITER, total_bytes, bytes_cycle);
