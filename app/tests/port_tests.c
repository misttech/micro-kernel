...
static void dump_port_result(const port_result_t *result) {
  ...
  LTRACEF("[%02x %02x %02x %02x %02x %02x %02x %02x]\n", p->value[0], p->value[1], p->value[2],
          p->value[3], p->value[4], p->value[5], p->value[6], p->value[7]);
}
...
static int two_threads_basic(void) {
  ...
  thread_t *t1 =
      thread_create("worker1", &ping_pong_thread, NULL, DEFAULT_PRIORITY, DEFAULT_STACK_SIZE);
  thread_t *t2 =
      thread_create("worker2", &ping_pong_thread, NULL, DEFAULT_PRIORITY, DEFAULT_STACK_SIZE);
  thread_resume(t1);
  thread_resume(t2);
...
typedef enum { ADD_PORT, QUIT } action_t;
...
static int group_basic(void) {
  ...
  thread_t *wt =
      thread_create("g_watcher", &group_watcher_thread, NULL, DEFAULT_PRIORITY, DEFAULT_STACK_SIZE);
  thread_resume(wt);
...
static int group_waiting(void) {
  ...
  thread_t *t1 =
      thread_create("receiver", &receive_thread, (void *)pg, DEFAULT_PRIORITY, DEFAULT_STACK_SIZE);
