...
static int fibo_thread(void *argv) {
  ...
  t[0] =
      thread_create(name, &fibo_thread, (void *)(fibo - 1), DEFAULT_PRIORITY, DEFAULT_STACK_SIZE);
  if (!t[0]) {
  ...
  t[1] =
      thread_create(name, &fibo_thread, (void *)(fibo - 2), DEFAULT_PRIORITY, DEFAULT_STACK_SIZE);
  if (!t[1]) {
    printf("error creating thread for fibo %ld\n", fibo - 2);
    thread_resume(t[0]);
