...
#define ITERATIONS \
  (256 * 1024 * 1024 / BUFFER_SIZE)  // enough iterations to have to copy/set 256MB of memory

#if 1
static inline void *mymemcpy(void *dest, const void *source, size_t len) {
...
static void *null_memcpy(void *dest, const void *source, size_t len) { return dest; }

static lk_time_t bench_memcpy_routine(void *memcpy_routine(void *, const void *, size_t),
...
static void bench_memcpy(void) {
  ...
  for (srcalign = 0; srcalign < 64;) {
    for (dstalign = 0; dstalign < 64;) {
      ...
      printf("c memcpy %u msecs, %llu bytes/sec; ", c,
             (uint64_t)BUFFER_SIZE * ITERATIONS * 1000ULL / c);
      printf("libc memcpy %u msecs, %llu bytes/sec; ", libc,
             (uint64_t)BUFFER_SIZE * ITERATIONS * 1000ULL / libc);
      printf("my memcpy %u msecs, %llu bytes/sec; ", mine,
...
static void bench_memset(void) {
  ...
  for (dstalign = 0; dstalign < 64; dstalign++) {
    ...
    printf("c memset %u msecs, %llu bytes/sec; ", c,
           (uint64_t)BUFFER_SIZE * ITERATIONS * 1000ULL / c);
    printf("libc memset %u msecs, %llu bytes/sec; ", libc,
           (uint64_t)BUFFER_SIZE * ITERATIONS * 1000ULL / libc);
    printf("my memset %u msecs, %llu bytes/sec; ", mine,
           (uint64_t)BUFFER_SIZE * ITERATIONS * 1000ULL / mine);
    printf("\n");
  }
}
