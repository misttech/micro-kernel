...
static status_t usb_register_cb(void *cookie, usb_callback_op_t op,
...
static bool handle_usb_cmd(const void *data, const size_t n, cmd_response_t *resp) {
  ...
  switch (header->cmd) {
    case USB_CMD_FLASH:
      ...
      while (image_length > 0) {
        ssize_t xfer =
            (image_length > (ssize_t)sizeof(buffer)) ? (ssize_t)sizeof(buffer) : image_length;
    ...
    case USB_CMD_DEVINFO:
      st = buildsig_search((void *)moot_system_info.sys_base_addr, DEFAULT_BUILDSIG_SEARCH_LEN,
                           1024 * 1024, &version);
      if (st != NO_ERROR) {
      ...
      snprintf((char *)buffer, buflen, "%s\n%s\n%s\n%s\n%s", version->arch, version->platform,
               version->target, version->project, version->buildid);
...
void init_usb_boot(void) { usb_register_callback(&usb_register_cb, NULL); }
