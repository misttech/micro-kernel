...
in_addr_t lookup_hostname(const char *hostname) {
  ...
  for (temp = info; temp; temp = temp->ai_next) {
    ...
    //          temp->ai_flags, temp->ai_family, temp->ai_socktype, temp->ai_protocol,
    //          temp->ai_addrlen);
...
int udp_connect(in_addr_t addr, unsigned port) { return inet_connect(addr, SOCK_DGRAM, port); }

int tcp_listen(in_addr_t addr, unsigned port) { return inet_listen(addr, SOCK_STREAM, port, 0); }

int tcp_connect(in_addr_t addr, unsigned port) { return inet_connect(addr, SOCK_STREAM, port); }
