...
void vmm_init_preheap(void) {
  ...
  arch_mmu_init_aspace(&_kernel_aspace.arch_aspace, KERNEL_ASPACE_BASE, KERNEL_ASPACE_SIZE,
...
static vmm_region_t *alloc_region_struct(const char *name, vaddr_t base, size_t size, uint flags,
                                         uint arch_mmu_flags) {
  DEBUG_ASSERT(name);
 ...
 */
static inline bool check_gap(vmm_aspace_t *aspace, vmm_region_t *prev, vmm_region_t *next,
                             vaddr_t *pva, vaddr_t align, size_t size, uint arch_mmu_flags) {
  ...
  *pva = arch_mmu_pick_spot(
      &aspace->arch_aspace, gap_beg, prev ? prev->arch_mmu_flags : ARCH_MMU_FLAG_INVALID, gap_end,
      next ? next->arch_mmu_flags : ARCH_MMU_FLAG_INVALID, align, size, arch_mmu_flags);
  if (*pva < gap_beg)
...
static vaddr_t alloc_spot(vmm_aspace_t *aspace, size_t size, uint8_t align_pow2,
  ...
  if (check_gap(aspace, NULL, list_peek_head_type(&aspace->region_list, vmm_region_t, node), &spot,
                align, size, arch_mmu_flags))
    goto done;

  /* search the middle of the list */
  list_for_every_entry (&aspace->region_list, r, vmm_region_t, node) {
    if (check_gap(aspace, r, list_next_type(&aspace->region_list, &r->node, vmm_region_t, node),
...
static vmm_region_t *alloc_region(vmm_aspace_t *aspace, const char *name, size_t size,
                                  vaddr_t vaddr, uint8_t align_pow2, uint vmm_flags,
                                  uint region_flags, uint arch_mmu_flags) {
...
status_t vmm_reserve_space(vmm_aspace_t *aspace, const char *name, size_t size, vaddr_t vaddr) {
  ...
  vmm_region_t *r = alloc_region(aspace, name, size, vaddr, 0, VMM_FLAG_VALLOC_SPECIFIC,
...
status_t vmm_alloc_physical(vmm_aspace_t *aspace, const char *name, size_t size, void **ptr,
                            uint8_t align_log2, paddr_t paddr, uint vmm_flags,
                            uint arch_mmu_flags) {
  status_t err;
 ...
 */
static status_t vmm_remove_region_locked(vmm_aspace_t *aspace, vaddr_t vaddr,
                                         vmm_region_t **r_out) {
  DEBUG_ASSERT(aspace);
  DEBUG_ASSERT(r_out);
  DEBUG_ASSERT(is_mutex_held(&vmm_lock));
...
status_t vmm_create_aspace(vmm_aspace_t **_aspace, const char *name, uint flags) {
  ...
  err =
      arch_mmu_init_aspace(&aspace->arch_aspace, aspace->base, aspace->size,
...
static void dump_region(const vmm_region_t *r) {
  printf("\tregion %p: name '%s' range 0x%lx - 0x%lx size 0x%zx flags 0x%x mmu_flags 0x%x\n", r,
         r->name, r->base, r->base + r->size - 1, r->size, r->flags, r->arch_mmu_flags);
}

static void dump_aspace(const vmm_aspace_t *a) {
  printf("aspace %p: name '%s' range 0x%lx - 0x%lx size 0x%zx flags 0x%x\n", a, a->name, a->base,
         a->base + a->size - 1, a->size, a->flags);
...
static int cmd_vmm(int argc, const console_cmd_args *argv) {
  ...
  if (!strcmp(argv[1].str, "aspaces")) {
    ...
    void *ptr = (void *)0x99;
    status_t err = vmm_alloc_physical(test_aspace, "physical test", argv[3].u, &ptr, argv[4].u,
    ...
    void *ptr = (void *)0x99;
    status_t err =
        vmm_alloc_contiguous(test_aspace, "contig test", argv[2].u, &ptr, argv[3].u, 0, 0);
