...
void add_to_bus_list(bus *b) { list_add_tail(&bus_list, b->list_node_ptr()); }
...
uint8_t allocate_next_bus() { return ++last_bus; }

uint8_t get_last_bus() { return last_bus; }
...
status_t pci_bus_mgr_find_device_by_class(pci_location_t *state, uint8_t base_class,
                                          uint8_t sub_class, uint8_t interface, size_t index) {
...
void pci_dump_bar(const pci_bar_t *bar, int index) {
  if (bar->addr >= UINT32_MAX || bar->size >= UINT32_MAX) {
    printf("BAR %d: addr %-#16llx size %-#16zx io %d 64b %d pref %d\n", index, bar->addr, bar->size,
           bar->io, bar->size_64, bar->prefetchable);
  } else {
