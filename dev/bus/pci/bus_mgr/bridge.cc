...
status_t bridge::probe(pci_location_t loc, bus *parent_bus, bridge **out_bridge) {
  ...
  *out_bridge = nullptr;
  ...
  LTRACEF("primary bus %hhd secondary %hhd subordinate %hhd\n", br->primary_bus(),
          br->secondary_bus(), br->subordinate_bus());
  ...
  *out_bridge = br;
  ...
  if (br->secondary_bus() == 0) {
    ...
    // we do not yet know the range of busses we will find downstream, lower level bridges will have
    // to back patch us as they find new children.
...
void bridge::dump(size_t indent) {
  ...
  printf("bridge %s %04hx:%04hx primary bus %d child busses [%d..%d]\n", pci_loc_string(loc_, str),
         vendor_id(), device_id(), primary_bus(), secondary_bus(), subordinate_bus());
  ...
  for (size_t b = 0; b < 2; b++) {
    if (bars_[b].valid) {
      ...
      printf("BAR %zu: addr %#" PRIx64 " size %#zx io %d valid %d\n", b, bars_[b].addr,
             bars_[b].size, bars_[b].io, bars_[b].valid);
    }
  }
...
status_t bridge::compute_bar_sizes_no_local_bar(bar_sizes *bridge_sizes) {
  ...
  auto perdev = [&](device *d) -> status_t {
    ...
    if (LOCAL_TRACE) {
      char str[14];
      printf("bar sizes for device %s: io %#x align %u mmio %#x align %u mmio64 %#" PRIx64
             " align %u prefetch %#" PRIx64 " align %u prefetch64 %#" PRIx64 " align %u\n",
             pci_loc_string(d->loc(), str), sizes.io_size, sizes.io_align, sizes.mmio_size,
             sizes.mmio_align, sizes.mmio64_size, sizes.mmio64_align, sizes.prefetchable_size,
             sizes.prefetchable_align, sizes.prefetchable64_size, sizes.prefetchable64_align);
    }
...
status_t bridge::get_bar_alloc_requests(list_node *bar_alloc_requests) {
  ...
  auto make_request = [this, &bar_alloc_requests](pci_resource_type type, bool prefetchable,
                                                  uint64_t size, uint8_t align) {
    if (size > 0) {
  ...
  make_request(PCI_RESOURCE_MMIO_RANGE, true, bus_sizes.prefetchable_size,
               bus_sizes.prefetchable_align);
  make_request(PCI_RESOURCE_MMIO64_RANGE, true, bus_sizes.prefetchable64_size,
               bus_sizes.prefetchable64_align);
...
status_t bridge::assign_resource(bar_alloc_request *request, uint64_t address) {
  ...
  // PROBLEM: do we recurse here and start the bus allocation for the children with a restricted
  // resource allocator but we have only a subset of resources to allocate. Or do we wait until the
  // bridge is completely configured, and then trigger a recursive assignment on each sub bus (this
