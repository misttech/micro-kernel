...
pci_bios32 *pci_bios32::detect() {
  ...
  LTRACEF("magic '%c%c%c%c' entry %p len %d checksum %#hhx\n", pci->magic[0], pci->magic[1],
          pci->magic[2], pci->magic[3], pci->entry, pci->length * 16, pci->checksum);
  ...
  __asm__(
      ...
      "xor %%ah,%%ah		\n"
      "1:"
      : "=a"(present), "=b"(version), "=c"(busses), "=d"(signature)
      : "0"(PCIBIOS_PRESENT), "D"(&b32_entry)
      : "cc", "memory");
  LTRACEF("PCI_BIOS_PRESENT returns present %#x, version %#x, busses %#x, signature %#x\n", present,
          version, busses, signature);
...
int pci_bios32::read_config_byte(const pci_location_t state, uint32_t reg, uint8_t *value) {
  ...
  __asm__(
      ...
      "1:"
      : "=c"(*value), "=a"(ret)
      : "1"(PCIBIOS_READ_CONFIG_BYTE), "b"(bx), "D"(reg), "S"(&bios32_entry_)
      : "cc", "memory");
  ret >>= 8;
  return ret & 0xFF;
}
...
int pci_bios32::read_config_half(const pci_location_t state, uint32_t reg, uint16_t *value) {
  ...
  __asm__(
      ...
      "1:"
      : "=c"(*value), "=a"(ret)
      : "1"(PCIBIOS_READ_CONFIG_WORD), "b"(bx), "D"(reg), "S"(&bios32_entry_)
      : "cc", "memory");
  ret >>= 8;
  return ret & 0xFF;
}
...
int pci_bios32::read_config_word(const pci_location_t state, uint32_t reg, uint32_t *value) {
  ...
  __asm__(
      ...
      "1:"
      : "=c"(*value), "=a"(ret)
      : "1"(PCIBIOS_READ_CONFIG_DWORD), "b"(bx), "D"(reg), "S"(&bios32_entry_)
      : "cc", "memory");
  ret >>= 8;
  return ret & 0xFF;
}
...
int pci_bios32::write_config_byte(const pci_location_t state, uint32_t reg, uint8_t value) {
  ...
  __asm__(
      ...
      "1:"
      : "=a"(ret)
      : "0"(PCIBIOS_WRITE_CONFIG_BYTE), "c"(value), "b"(bx), "D"(reg), "S"(&bios32_entry_)
      : "cc", "memory");
  ret >>= 8;
  return ret & 0xFF;
}
...
int pci_bios32::write_config_half(const pci_location_t state, uint32_t reg, uint16_t value) {
  ...
  __asm__(
      ...
      "1:"
      : "=a"(ret)
      : "0"(PCIBIOS_WRITE_CONFIG_WORD), "c"(value), "b"(bx), "D"(reg), "S"(&bios32_entry_)
      : "cc", "memory");
  ret >>= 8;
  return ret & 0xFF;
}
...
int pci_bios32::write_config_word(const pci_location_t state, uint32_t reg, uint32_t value) {
  ...
  __asm__(
      ...
      "1:"
      : "=a"(ret)
      : "0"(PCIBIOS_WRITE_CONFIG_DWORD), "c"(value), "b"(bx), "D"(reg), "S"(&bios32_entry_)
      : "cc", "memory");
  ret >>= 8;
  return ret & 0xFF;
}
