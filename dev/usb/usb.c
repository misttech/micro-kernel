          ...
          if ((setup->request_type & RECIP_MASK) == RECIP_DEVICE) {
            ...
            switch (setup->value) {
              case 0x100: /* device */
                ...
                usbc_ep0_send(speed->device.desc, speed->device.len, setup->length);
                break;
              case 0x200: /* CONFIGURATION */
                LTRACEF("got GET_DESCRIPTOR, config descriptor\n");
                usbc_ep0_send(speed->config.desc, speed->config.len, setup->length);
                break;
              case 0x300: /* Language ID */
                LTRACEF("got GET_DESCRIPTOR, language id\n");
                usbc_ep0_send(usb.config->langid.desc, usb.config->langid.len, setup->length);
                break;
              case (0x301)...(0x3ff): {
                ...
                for (i = 0; i < MAX_STRINGS; i++) {
                  if (usb.strings[i].id == id) {
                    usbc_ep0_send(usb.strings[i].string.desc, usb.strings[i].string.len,
                                  setup->length);
                    found = true;
                    break;
                  }
              ...
              case 0x600: /* DEVICE QUALIFIER */
                ...
                usbc_ep0_send(speed->device_qual.desc, speed->device_qual.len, setup->length);
                break;
              case 0xa00:
...
static void usb_init(uint level) { list_initialize(&usb.cb_list); }
