...
static enum handler_return virtio_net_irq_driver_callback(struct virtio_device *dev, uint ring,
...
static void dump_feature_bits(uint64_t feature) {
  ...
  if (feature & VIRTIO_NET_F_CSUM)
    printf(" CSUM");
  if (feature & VIRTIO_NET_F_GUEST_CSUM)
    printf(" GUEST_CSUM");
  if (feature & VIRTIO_NET_F_CTRL_GUEST_OFFLOADS)
    printf(" CTRL_GUEST_OFFLOADS");
  if (feature & VIRTIO_NET_F_MTU)
    printf(" MTU");
  if (feature & VIRTIO_NET_F_MAC)
    printf(" MAC");
  if (feature & VIRTIO_NET_F_GSO)
    printf(" GSO");
  if (feature & VIRTIO_NET_F_GUEST_TSO4)
    printf(" GUEST_TSO4");
  if (feature & VIRTIO_NET_F_GUEST_TSO6)
    printf(" GUEST_TSO6");
  if (feature & VIRTIO_NET_F_GUEST_ECN)
    printf(" GUEST_ECN");
  if (feature & VIRTIO_NET_F_GUEST_UFO)
    printf(" GUEST_UFO");
  if (feature & VIRTIO_NET_F_HOST_TSO4)
    printf(" HOST_TSO4");
  if (feature & VIRTIO_NET_F_HOST_TSO6)
    printf(" HOST_TSO6");
  if (feature & VIRTIO_NET_F_HOST_ECN)
    printf(" HOST_ECN");
  if (feature & VIRTIO_NET_F_HOST_UFO)
    printf(" HOST_UFO");
  if (feature & VIRTIO_NET_F_MRG_RXBUF)
    printf(" MRG_RXBUF");
  if (feature & VIRTIO_NET_F_STATUS)
    printf(" STATUS");
  if (feature & VIRTIO_NET_F_CTRL_VQ)
    printf(" CTRL_VQ");
  if (feature & VIRTIO_NET_F_CTRL_RX)
    printf(" CTRL_RX");
  if (feature & VIRTIO_NET_F_CTRL_VLAN)
    printf(" CTRL_VLAN");
  if (feature & VIRTIO_NET_F_GUEST_ANNOUNCE)
    printf(" GUEST_ANNOUNCE");
  if (feature & VIRTIO_NET_F_MQ)
    printf(" MQ");
  if (feature & VIRTIO_NET_F_CTRL_MAC_ADDR)
    printf(" CTRL_MAC_ADDR");
  if (feature & VIRTIO_NET_F_HASH_TUNNEL)
    printf(" HASH_TUNNEL");
  if (feature & VIRTIO_NET_F_VQ_NOTF_COAL)
    printf(" VQ_NOTF_COAL");
  if (feature & VIRTIO_NET_F_NOTF_COAL)
    printf(" NOTF_COAL");
  if (feature & VIRTIO_NET_F_GUEST_USO4)
    printf(" GUEST_USO4");
  if (feature & VIRTIO_NET_F_GUEST_USO6)
    printf(" GUEST_USO6");
  if (feature & VIRTIO_NET_F_HOST_USO)
    printf(" HOST_USO");
  if (feature & VIRTIO_NET_F_HASH_REPORT)
    printf(" HASH_REPORT");
  if (feature & VIRTIO_NET_F_GUEST_HDRLEN)
    printf(" GUEST_HDRLEN");
  if (feature & VIRTIO_NET_F_RSS)
    printf(" RSS");
  if (feature & VIRTIO_NET_F_RSC_EXT)
    printf(" RSC_EXT");
  if (feature & VIRTIO_NET_F_STANDBY)
    printf(" STANDBY");
  if (feature & VIRTIO_NET_F_SPEED_DUPLEX)
    printf(" SPEED_DUPLEX");
  printf("\n");
}
...
status_t virtio_net_init(struct virtio_device *dev) {
  ...
  uint64_t host_features =
      virtio_read_host_feature_word(dev, 0) | (uint64_t)virtio_read_host_feature_word(dev, 1) << 32;
...
status_t virtio_net_start(void) {
  ...
  thread_resume(thread_create("virtio_net_rx", &virtio_net_rx_worker, (void *)the_ndev,
...
int virtio_net_found(void) { return the_ndev ? 1 : 0; }
