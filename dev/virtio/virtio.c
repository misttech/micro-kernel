...
int virtio_mmio_detect(void *ptr, uint count, const uint irqs[], size_t stride) {
  ...
  for (uint i = 0; i < count; i++) {
    volatile struct virtio_mmio_config *mmio =
        (struct virtio_mmio_config *)((uint8_t *)ptr + i * stride);
    ...
    LTRACEF("looking at %p: magic 0x%x version 0x%x did 0x%x vid 0x%x\n", mmio, mmio->magic,
            mmio->version, mmio->device_id, mmio->vendor_id);
...
void virtio_reset_device(struct virtio_device *dev) { dev->mmio_config->status = 0; }
