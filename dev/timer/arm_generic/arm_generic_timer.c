...
lk_bigtime_t current_time_hires(void) { return cntpct_to_lk_bigtime(read_cntpct()); }

lk_time_t current_time(void) { return cntpct_to_lk_time(read_cntpct()); }

static uint32_t abs_int32(int32_t a) { return (a > 0) ? a : -a; }
...
static void test_cntpct_to_lk_time(uint32_t cntfrq, lk_time_t expected_lk_time,
                                   uint32_t wrap_count) {
  lk_time_t lk_time;
  uint64_t cntpct;
...
static void test_cntpct_to_lk_bigtime(uint32_t cntfrq, uint64_t expected_s) {
  ...
  test_time_conversion_check_result(lk_bigtime, expected_lk_bigtime,
                                    (1000 * 1000 + cntfrq - 1) / cntfrq, false);
