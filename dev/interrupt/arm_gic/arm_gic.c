...
#else
static bool arm_gic_interrupt_change_allowed(uint irq) { return true; }
...
LK_INIT_HOOK_FLAGS(arm_gic_init_percpu, arm_gic_init_percpu, LK_INIT_LEVEL_PLATFORM_EARLY,
                   LK_INIT_FLAG_SECONDARY_CPUS);

static void arm_gic_suspend_cpu(uint level) { suspend_resume_fiq(false, false); }

LK_INIT_HOOK_FLAGS(arm_gic_suspend_cpu, arm_gic_suspend_cpu, LK_INIT_LEVEL_PLATFORM,
                   LK_INIT_FLAG_CPU_SUSPEND);
...
LK_INIT_HOOK_FLAGS(arm_gic_resume_cpu, arm_gic_resume_cpu, LK_INIT_LEVEL_PLATFORM,
                   LK_INIT_FLAG_CPU_RESUME);

static uint arm_gic_max_cpu(void) { return (gicreg_read32(0, GICD_TYPER) >> 5) & 0x7; }

static status_t gic_configure_interrupt(unsigned int vector, enum interrupt_trigger_mode tm,
...
static status_t arm_gic_set_secure_locked(u_int irq, bool secure) {
  ...
  LTRACEF("irq %d, secure %d, GICD_IGROUP%d = %x\n", irq, secure, reg,
          gicreg_read32(0, GICD_IGROUPR(reg)));
#endif
  return NO_ERROR;
}
...
static status_t arm_gic_set_target_locked(u_int irq, u_int cpu_mask, u_int enable_mask) {
  ...
  LTRACEF("irq %i, GICD_ITARGETSR%d %x => %x (got %x)\n", irq, reg, old_val, new_val,
          gicreg_read32(0, GICD_ITARGETSR(reg)));
...
static status_t arm_gic_set_priority_locked(u_int irq, uint8_t priority) {
  ...
  LTRACEF("irq %i, new GICD_IPRIORITYR%d = %x, req %x\n", irq, reg,
          gicreg_read32(0, GICD_IPRIORITYR(reg)), regval);
